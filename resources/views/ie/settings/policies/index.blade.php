...
@section('page-title')
    {{-- PC --}}
    <h2 class="hidden-xs hidden-sm"> <x-get-program-code url="/ie/settings/categories" /> {{ $category->name }} : {{ $sub_category->name }} : Policies</h2>
    <ol class="breadcrumb hidden-xs hidden-sm">
        ...
        <li class="breadcrumb-item active">
            <strong>{{ $sub_category->name }} : policy</strong>
        </li>
    </ol>
    {{-- MOBILE --}}
    <h3 class="m-t-md m-b-sm show-xs-only show-sm-only">
        {{ $sub_category->name }} : policy <br>
                ...
                <div class="col-md-6 b-r">
                    ...
                    @if($policy_expense)
                        ...
                        <div class="p-xl pm-xl text-center" style="min-height: 200px">
                            ...
                            <div>
                                <a href="{{ route('ie.settings.policy.rates.index',[$category->category_id,$sub_category->sub_category_id,$policy_expense->policy_id]) }}" class="btn btn-resize btn-success">
                    ...
                    @if($policy_mileage)
                        ...
                        <div class="p-xl pm-xl text-center" style="min-height: 200px">
                            ...
                            <div>
                                ...
                                <a href="{{ route('ie.settings.policies.rates.index',[$category->category_id,$sub_category->sub_category_id,$policy_mileage->policy_id]) }}" class="btn btn-resize btn-primary">
...
@include('ie.settings.policy._modal_use_policy')
    ...
    <script>
        $(document).ready(function(){
            ...
            function showModalUsePolicy(type){
                ...
                $.ajax({
                    url: "{{ url('/') }}/ie/settings/categories/{{ $category->category_id }}/sub-categories/{{ $sub_category->sub_category_id }}/policy/create",
                    type: 'GET',
                    data: { type : type },
            ...
            $("[id^='btn_disable_']").click(function(){
                ...
                function(isConfirm){
                    if (isConfirm) {
                        $.ajax({
                          type: "post",
                          url:  "{{ url('/') }}/ie/settings/categories/{{ $category->category_id }}/sub-categories/{{ $sub_category->sub_category_id }}/policy/"+id+"/inactive",
                          data: {
                            _token: "{{ csrf_token() }}",
                          },
                          beforeSend: function() {
