...
@if ( (\Auth::user()->isChangeApprover() || !!collect($approvers)->where('user_id', \Auth::user()->user_id)->count()) && $cashAdvance->next_approver )
    ...
    <div class="modal fade" id="modal-change-approver" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog pt-0 modal-md" role="document">
            ...
            <div class="modal-content">
                ...
                {!! Form::open(['route' => ['ie.cash-advances.change_approver', $cashAdvance->cash_advance_id], 
                    'method' => 'POST',
                        ...
                        <div class="clearfix">
                            <select name="select_approver" id="txt_select_approver" class="form-control select2">
                                @foreach ($approvers as $user)
                                    <option value="{{ $user->user_id }}" {{ $cashAdvance->next_approver_id == $user->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
