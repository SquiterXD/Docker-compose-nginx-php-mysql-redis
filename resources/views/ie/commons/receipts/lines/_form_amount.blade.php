...
<div id="div_alert_form_amount" class="alert alert-warning d-none">
    <ul id="ul_alert_form_amount" style="margin-bottom: 0">
...
<div class="tabs-container tab-form-receipt-line-amount">
    ...
    @if($policyExpense)
        <li class="active">
            <a data-toggle="tab" data-policy-id="{{ $policyExpense->policy_id }}" data-rate-id="{{ $rateExpense->policy_rate_id }}" 
    ...
    @if($policyMileage)
        <li class="{{ !$policyExpense ? 'active':'' }}">
            <a data-toggle="tab" data-policy-id="{{ $policyMileage->policy_id }}" data-rate-id="{{ $rateMileage->policy_rate_id }}" 
                    ...
                    <div class="col-md-3" style="padding-left: 5px;padding-right: 5px;">
                        <div class="m-b-sm">
                            ...
                            <div class="input-group">
                                ...
                                {!! Form::text('uom', isset($receiptLine) ? $receiptLine->uom : $subCategory->unit, ['class'=>'form-control input-sm', 'maxlength' => '50']) !!}
                                @if($subCategory->use_second_unit)
                                    ...
                                    {!! Form::text('second_uom', isset($receiptLine) ? $receiptLine->second_uom : $subCategory->second_unit, ['class'=>'form-control input-sm', 'maxlength' => '50']) !!}
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        <div id="tab-mileage" class="tab-pane {{ !$policyExpense ? 'active':'' }}">
            <div class="panel-body">
                ...
                <div class="row m-b-xs">
                    <div id="div_odermeter_reading" class="col-sm-5 d-none">
                        <div>
                            <label>
                ...
                <div class="row">
                    <div class="col-md-5 offset-md-1">
                        <div class="m-b-sm">
                            ...
                            <div class="input-group">
                                {!! Form::text('actual_quantity', $defaultQuantity, ['class'=>'form-control input-sm text-right']) !!}
                                {!! Form::text('uom', isset($receiptLine) ? $receiptLine->uom : $subCategory->unit, ['class'=>'form-control input-sm', 'maxlength' => '50']) !!}
                                @if($subCategory->use_second_unit)
                                    {!! Form::text('actual_second_quantity', $defaultSecondQuantity, ['class'=>'form-control text-right input-sm']) !!}
                                    {!! Form::text('second_uom', isset($receiptLine) ? $receiptLine->second_uom : $subCategory->second_unit, ['class'=>'form-control input-sm', 'maxlength' => '50']) !!}
                                @endif
                            </div>
                        </div>
                            ...
                            <div>
                                @if ($receiptType == 'CASH-ADVANCE')
                                    <label for="">
                                        ...
                                        <div>
                                            <small>ยอดเงินรวม</small>
                                        </div>
                                    </label>
                                @else 
                                    <label for="">
                                        <div>
                                            Amount <small>before VAT</small> <span class="text-danger">*</span>
                                        </div>
                                        <div>
                                            <small>ยอดเงินไม่รวมภาษีมูลค่าเพิ่ม</small>
                                        </div>
                                    </label>
                                @endif
                            </div>
                            <div class="input-group">
                                {!! Form::text('actual_amount', $defaultAmount, ['class'=>'form-control input-sm text-right',"autocomplete" => "off", "readonly" => !$parent->canImportantEditData()]) !!}
                                <span class="input-group-addon" style="padding-right: 5px;padding-left: 5px;">
                                    {{ $receipt->currency_id }}
                                </span>
                            </div>
                ...
                <div class="row">
                    @if ($receiptType == 'REIMBURSEMENT')
                        <div class="col-md-5 offset-md-1">
                            ...
                            {!! Form::select('actual_vat_id', [''=>'-'] + $VATLists, $defaultVatId,['class'=>'form-control text-right select2','id'=>'ddl_vat_id']) !!}
                        ...
                        <div class="col-md-5">
                            ...
                            {!! Form::select('actual_wht_id', [''=>'-'] + $WHTLists, $defaultWhtId,['class'=>'form-control text-right select2','id'=>'ddl_wht_id']) !!}
                            {{-- {!! Form::hidden('actual_wht_id', $defaultWhtId) !!} --}}
                        </div>
                    @elseif ($receiptType == 'CLEARING')
...
@if(!isset($receiptLine))
    ...
    {!! Form::hidden('wht_amount', null, 
        ['id'=>'txt_receipt_line_wht_amount']) !!}
@else
    ...
    {!! Form::hidden('amount_inc_vat', $receiptLine->amount_inc_vat, 
        ['id'=>'txt_receipt_line_amount_inc_vat']) !!}
    {!! Form::hidden('wht_amount', $receiptLine->wht_amount, 
        ['id'=>'txt_receipt_line_wht_amount']) !!}
@endif
    ...
    <div class="row m-t-sm">
        <div class="col-md-5 offset-md-1">
            <div class="m-b-sm">
                ...
                @if(!isset($receiptLine))
                    {!! Form::text('primary_vat_amount', null, ['class'=>'form-control text-right','style'=>'font-size: 18px;']) !!}
                    {{-- {!! Form::text('show_primary_vat_amount', null, ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_vat_amount', null, ['id'=>'txt_receipt_line_primary_vat_amount']) !!} --}}
                @else
                    {!! Form::text('primary_vat_amount', $receiptLine->primary_vat_amount ?? 0, ['class'=>'form-control text-right','style'=>'font-size: 18px;']) !!}
                    {{-- {!! Form::text('show_primary_vat_amount', number_format($receiptLine->primary_vat_amount, 2, '.', ','), ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_vat_amount', $receiptLine->primary_vat_amount, ['id'=>'txt_receipt_line_primary_vat_amount']) !!} --}}
                @endif
            </div>
        </div>
        <div class="col-md-5">
            <div class="m-b-sm">
                <div><label for="">
                    <div>Wht Amount <span class="text-danger">*</span></div>
                    <div><small>ภาษีหัก ณ ที่จ่าย</small></div>
                </label></div>
                @if(!isset($receiptLine))
                    {!! Form::text('show_primary_wht_amount', null, ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_wht_amount', null, ['id'=>'txt_receipt_line_primary_wht_amount']) !!}
                @else
                    {!! Form::text('show_primary_wht_amount', number_format($receiptLine->primary_wht_amount, 2, '.', ','), ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_wht_amount', $receiptLine->primary_wht_amount, ['id'=>'txt_receipt_line_primary_wht_amount']) !!}
                @endif
            </div>
        </div>
    </div>
    <div class="row m-t-sm">
        <div class="col-md-5 offset-md-1">
            <div class="m-b-sm">
                ...
                @if(!isset($receiptLine))
                    {!! Form::text('show_primary_amount', null, ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_amount', null, ['id'=>'txt_receipt_line_primary_amount']) !!}
                @else
                    {!! Form::text('show_primary_amount', number_format($receiptLine->primary_amount, 2, '.', ','), ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                    {!! Form::hidden('primary_amount', $receiptLine->primary_amount, ['id'=>'txt_receipt_line_primary_amount']) !!}
                @endif
            </div>
        </div>
        <div class="col-md-5">
            <div class="m-b-sm">
                ...
                <div class="input-group">
                    @if(!isset($receiptLine))
                        {!! Form::text('show_primary_amount_inc_vat', null, ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                        {!! Form::hidden('primary_amount_inc_vat', null, ['id'=>'txt_receipt_line_primary_amount_inc_vat']) !!}
                    @else
                        {!! Form::text('show_primary_amount_inc_vat', number_format($receiptLine->primary_amount_inc_vat, 2, '.', ','), ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                        {!! Form::hidden('primary_amount_inc_vat', $receiptLine->primary_amount_inc_vat, ['id'=>'txt_receipt_line_primary_amount_inc_vat']) !!}
                    @endif
            ...
            <div class="col-md-3">
                <div class="m-b-sm">
                    ...
                    @if(!isset($receiptLine))
                        ...
                        {!! Form::hidden('primary_vat_amount', null, ['id'=>'txt_receipt_line_primary_vat_amount']) !!} --}}
                    @else
                        {!! Form::text('primary_vat_amount', $receiptLine->primary_vat_amount ?? 0, ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                        {{-- {!! Form::text('show_primary_vat_amount', number_format($receiptLine->primary_vat_amount, 2, '.', ','), ['class'=>'form-control text-right','style'=>'font-size: 18px;','readonly'=>'readonly']) !!}
                        {!! Form::hidden('primary_vat_amount', $receiptLine->primary_vat_amount, ['id'=>'txt_receipt_line_primary_vat_amount']) !!} --}}
                    @endif
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var WHTs = jQuery.parseJSON(JSON.stringify({!! $WHTs->toJson() !!}));
		var whtFlag = "{{ $subCategory->wht_flag == '1' }}";
        ...
        $(formId).find("input[name='cal_mileage_by']").change(function(){
            ...
            if(calMileageBy == 'distance_traveled'){
                ...
                $("#div_odermeter_reading").addClass('d-none');
                ...
                $("#div_odermeter_reading").removeClass('d-none');
        ...
        {
            $(formId).find("input[name='actual_quantity']").focus(function(){
                this.value = '';
            });
            $(formId).find("input[name='actual_amount']").focus(function(){
                this.value = '';
            });
            ...
            $(formId).find("input[name='actual_amount']").blur(function(){
                this.value = whtFlag ? (-1) * Math.abs(this.value) : this.value;
                calActual();
            });
        ...
        function calActual()
        {
            ...
            // }
            var whtAmount = calWHTAmount(actualAmount, actualWHTId).toFixed(2);
            var primaryWHTAmount = (parseFloat(whtAmount) * exchangeRate).toFixed(2);
            ...
            $(formId).find("input[name='show_wht_amount']").val(numberWithCommas(whtAmount));
            $(formId).find("input[name='wht_amount']").val(whtAmount);
            $(formId).find("input[name='show_primary_wht_amount']").val(numberWithCommas(primaryWHTAmount));
            $(formId).find("input[name='primary_wht_amount']").val(primaryWHTAmount);
            // CALCULATE PRIMARY AMOUNT
            calPrimaryAmount(actualAmount,actualAmountIncVat);
            // calVATAmount(actualAmount,actualVATId);
            setInputAmount(actualQuantity,actualSecondQuantity,actualAmount, actualVATId, actualAmountIncVat, actualWHTId);
        ...
        function calAmount(amount, vatId)
        {
            if($.isNumeric(amount)) {
                ...
                var vatAmount = (parseFloat(amount) * (parseFloat(vatRate)/100)) + 0.00000001;
        ...
        function calWHTAmount(amount, whtId)
        {
            if($.isNumeric(amount)) {
                // var whtRate = getWHTRate(whtId);
                // var whtAmount = parseFloat(amount) * (parseFloat(whtRate)/100);
                ...
                // return (parseFloat(amount) + whtAmount + whtAmount).toFixed(2);
                return whtAmount;
            }
            return 0;
        }
        ...
        function calPrimaryAmount(amount,amountIncVat,mileage)
        {
            ...
            $(formId).find("input[name='show_primary_amount']").val(numberWithCommas(primaryAmount));
            $(formId).find("input[name='primary_amount']").val(primaryAmount);
            $(formId).find("input[name='show_primary_amount_inc_vat']").val(numberWithCommas(primaryAmountIncVat));
            $(formId).find("input[name='primary_amount_inc_vat']").val(primaryAmountIncVat);
            // $(formId).find("input[name='show_primary_vat_amount']").val(numberWithCommas(primaryVatAmount));
        ...
        function calAmountIncVatByPrimaryVatAmount(amount,primaryAmount,primaryVatAmount,selectedTabType)
        {
            ...
            $(formId).find("input[name='show_amount_inc_vat']").val(numberWithCommas(amountIncVat));
        ...
        function setInputAmount(quantity,secondQuantity,amount, vatId, amountIncVat, whtId)
        {
        ...
        {
            $(formId).find("input[name='uom']").attr('disabled','disabled');
            $(formId).find("input[name='second_uom']").attr('disabled','disabled');
        ...
        {
            $(formId).find("input[name='uom']").removeAttr('disabled','disabled');
            $(formId).find("input[name='second_uom']").removeAttr('disabled','disabled');
