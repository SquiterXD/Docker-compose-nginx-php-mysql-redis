...
@section('page-title')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">
            <strong><x-get-program-code url="/pm/settings/print-item-setup" />: แก้ไขกำหนดสีแบรนด์และผลิตภัณฑ์สำหรับการวางแผนรายวัน</strong>
        </li>
    </ol>
@stop
...
@section('content')
    ...
    <div class="ibox">
        ...
        <div class="ibox-title">
            <h5><x-get-program-code url="/pm/settings/print-item-setup" />: แก้ไขกำหนดสีแบรนด์และผลิตภัณฑ์สำหรับการวางแผนรายวัน</h5>
        </div>

        <div class="ibox-content qm-content">
            {!! Form::open(['route' => ['pm.settings.print-item-setup.update'], 'method' => 'get', 'autocomplete' => 'off', 'class' => 'form-horizontal']) !!}
            <print-item-setup-edit      :print-item-setup = "{{ json_encode($printItemSetup) }}"
                                        :item-number-list = "{{ json_encode($itemNumberList) }}"
                                        :size-list = "{{ json_encode($sizeList) }}"
                                        :print-type-list = "{{ json_encode($printTypeList) }}">
            </print-item-setup-edit>
