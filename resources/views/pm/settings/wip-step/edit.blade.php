        ...
        <div class="col-lg-12">
            <div class="ibox">
                ...
                <div class="ibox-content">
                    {{-- {!! Form::open(['route' => ['pm.settings.wip-step.update', $routing->routing_id] , "method" => "post" , "autocomplete" => "off", 'class' => 'form-horizontal']) !!}
                    @method('PUT') --}}
                    <wip-form 
                        :organizations="{{ json_encode($organizations) }}"
                        :uoms="{{ json_encode($uoms) }}"
                        :open-class="{{ json_encode($openClass) }}"
                        :default-value="{{ json_encode($routing)}}"
                        :old="{{ json_encode(Session::getOldInput()) }}"
                        :url-save="{{ json_encode(route('pm.settings.wip-step.update', $routing->routing_id)) }}"
