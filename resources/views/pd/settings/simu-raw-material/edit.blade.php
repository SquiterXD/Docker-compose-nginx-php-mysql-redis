        ...
        <div class="col-lg-12">
            <div class="ibox">
                ...
                <div class="ibox-content">
                    {!! Form::open(['route' => ['pd.settings.simu-raw-material.update', $simuRaw->simu_raw_id] , "method" => "post" , "autocomplete" => "off", 'class' => 'form-horizontal']) !!}
                        @method('PUT')                        
                      
                        <simu-form  :simulation-types="{{ json_encode($simulationTypes) }}" 
                                    :uoms="{{ json_encode($uoms) }}"
                                    :default-value="{{ json_encode($simuRaw) }}"
                                    :old="{{ json_encode(Session::getOldInput()) }}"
                                    :check-transection="{{ json_encode($checkTransection) }}">
                        </simu-form>
