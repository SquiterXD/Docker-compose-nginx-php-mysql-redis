    ...
    <table>
        <thead>
            ...
            <tr>
                ...
                <td align="center" colspan=" 6" style="{{ $font18 }}"><b> กระดาษทำการคำนวณค่าวัตถุดิบ-คิดเข้างาน - แยกตามคำสั่งผลิต </b></td>
                <td style="{{ $font18 }}"></td>
            </tr>
            <tr>
            ...
            @foreach ($datas_level_1->groupBy('batch_no') as $batchNo => $dataByBatchs)
                ...
                @foreach ($dataByBatchs->sortBy('wip_step')->groupBy('wip_step') as $wip => $dataByWip)
                    ...
                    <tr>
                        ...
                        <td style="{{ $styleFont16 }}"></td>
                        <td style="{{ $styleFont16 }}"></td>
                        <td style="{{ $styleFont16 }}" align="right"><b> {{ $dataWip->transaction_quantity }} </b></td> {{-- output --}}
                        <td style="{{ $styleFont16 }}"></td>
                        <td style="{{ $styleFont16 }}"></td>
                        <td style="{{ $styleFont16 }}"></td>
                    </tr>
                    @foreach ($datas_level_2->where('batch_no', $batchNo)->where('wip_step', $wip)->sortBy('tobacco_group_code')->groupBy('tobacco_group_code') as $tobaccoGroupCode => $dataByGroupCode)
                        ...
                        @foreach ($dataByGroupCode->sortBy('item_number') as $data)
                            ...
                            @if ($loop->last)
                                ...
                                @endphp
                                <tr>
                                    <td colspan="2" align="center" style="{{ $styleFont16 }}"><b>รวม {{ $tobaccoGroupCode }} {{ optional($dataGroupCode)->tobacco_group }}</b> </td>
                                    <td style="{{ $styleFont16 }}"></td>
                                    <td style="{{ $styleFont16 }}"></td>
                                    <td style="{{ $styleFont16 }}"></td>
                ...
                @endforeach
                <tr>
                    <td colspan="2" align="center" style="{{ $styleFont16 }}"><b>รวมตามคำสั่งผลิต {{ $batchNo }}</b> </td>
                    <td style="{{ $styleFont16 }}"></td>
                    <td style="{{ $styleFont16 }}"></td>
                    <td style="{{ $styleFont16 }}"></td>
                    <td style="{{ $styleFont16 }}" align="right"><b> {{ $sumBatchAQ }}  </b> </td>
                    <td style="{{ $styleFont16 }}"></td>
                    <td style="{{ $styleFont16 }}" align="right"><b> {{ $sumBatchAQSP }} </b> </td>
                </tr>
            @endforeach
        </tbody>
    </table>
