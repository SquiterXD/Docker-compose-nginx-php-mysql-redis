                ...
                <tbody>
                    ...
                    @foreach ($dataByDate->groupBy('oprn_code') as $wip => $dataByWip)
                        @php
                            ...
                            $confirmQty = productWipV($dataByDate[0]->batch_id, $wip, $date)->sum('confirm_qty');
                            $endingQty = productWipV($dataByDate[0]->batch_id, $wip, $date)->sum('ending_qty');
                            $lossQty = productWipV($dataByDate[0]->batch_id, $wip, $date)->sum('loss_qty');
                            ...
                            /////
                            // dd($dataByDate[0]->batch_id);
                            $sumRawMaterial = 0;
                            $productWipV = productWipV($dataByDate[0]->batch_id, $wip, $date);
                            // dd($productWipV , $dataByWip[0]->cost_quantity);
                            $receiveWip =  $endingQty / $dataByWip[0]->cost_quantity;
                            ...
                            $c  =  ($confirmQty  * ($receiveWip)) == 0 
                                    ? 0
                                    :  ($confirmQty  * ($receiveWip)) / $dataByWip->sum('sum_raw_material');

                            $g =    $dataByWip->sum('sum_raw_material') +
                                    ($confirmQty * $wageCostAmount) +
                                    ($confirmQty * $varyCostAmount) +
                                    ($confirmQty * $fixedCost);

                            // $tranferQty =   $productWipV->transfer_qty / $dataByWip[0]->cost_quantity; เแลี่ยนเป็น confirm_qty
                            // dd($confirmQty , $dataByWip[0]->cost_quantity);
                            $tranferQty =   $confirmQty / $dataByWip[0]->cost_quantity;
                            // dd($tranferQty );    
                            // if(!$tranferQty){
                            //     dd($dataByWip , productWipV($dataByDate[0]->batch_id, $wip, $date), $confirmQty , $dataByDate[0]->batch_id, $wip, $date);
                            // }
                            $sumTranfer = (($dataByWip->sum('sum_raw_material') + $c) * $tranferQty) / ($confirmQty + $receiveWip) +
                                            ($tranferQty * $wageCostAmount)+
                                            ($tranferQty * $varyCostAmount)+
                                            ($tranferQty * $fixedCost);

                            // $tranferWip = $transferQty  /$dataByWip[0]->cost_quantity;
                            $tranferWip = $confirmQty  /$dataByWip[0]->cost_quantity;
                            
                            $tranferData[$wip]['tranfer_qty']        =  $tranferQty;
                            $tranferData[$wip]['tranfer_raw_mat']    =  (($dataByWip->sum('sum_raw_material') + $c) * $tranferQty) / ($confirmQty + $receiveWip);
                        ...
                        <tr>
                            ...
                            <td style="{{ $styleFont16 }}"> <b> {{ $confirmQty }} </b></td>
                            <td style="{{ $styleFont16 }}"> <b> {{ $dataByWip->sum('sum_raw_material') }}  </b></td>
                            <td style="{{ $styleFont16 }}"> <b> {{ $confirmQty * $wageCostAmount }}  </b></td>
                            <td style="{{ $styleFont16 }}"> <b> {{ $confirmQty * $varyCostAmount }} </b> </td>
                            <td style="{{ $styleFont16 }}"> <b> {{ $confirmQty * $fixedCost }} </b> </td>
                        ...
                        @if (!$loop->last)
                            <tr>
                                ...
                                <td style="{{ $styleFont16 }}"><b> {{ $confirmQty + $receiveWip }} </b></td>
                            ...
                            <tr>
                                ...
                                <td style="{{ $styleFont16 }}"> <b> {{ (($dataByWip->sum('sum_raw_material') + $c) * $tranferQty) / ($confirmQty + $receiveWip)   }}  </b> </td>
                            ...
                            <tr>
                                ...
                                <td style="{{ $styleFont16 }}"> <b> {{ (($dataByWip->sum('sum_raw_material') + $c) * $tranferWip) / ($confirmQty + $receiveWip) }}   </b></td>
                                ...
                                <td style="{{ $styleFont16 }}"> <b> {{ 
                                    (($dataByWip->sum('sum_raw_material') + $c) * $tranferWip) / ($confirmQty + $receiveWip) + 
                        ...
                        @endif
                        @if ($loop->last)
                        @if (count($dataByDate->groupBy('oprn_code')) == 1)
                            <tr>
                                <td style="{{ $styleFont16 }}"><b> ต้นทุนสินค้าสำเร็จรูป  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ $confirmQty  }}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ $dataByWip->sum('sum_raw_material')   + 0}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $wageCostAmount )  + 0}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $varyCostAmount )  + 0}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $fixedCost) + 0}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ $g + 0 }} </b> </td>
                            </tr>
                        @else
                            ...
                            <tr>
                                ...
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $wageCostAmount )  + $tranferData[$dataByWip[0]->previous_wip]['tranfer_wage']}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $varyCostAmount )  + $tranferData[$dataByWip[0]->previous_wip]['tranfer_vary']}}  </b></td>
                                <td style="{{ $styleFont16 }}"><b> {{ ($confirmQty * $fixedCost) + $tranferData[$dataByWip[0]->previous_wip]['tranfer_fixed']}}  </b></td>
