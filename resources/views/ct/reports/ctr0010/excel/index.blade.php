    ...
    <table >
        @foreach ($datas->groupBy('product_item_number') as $productItemNumber => $groupProducts)
                    ...
                    <table  width="100%" >
                        <thead>
                            <tr>
                                ...
                                <td width="33%"  align="center" colspan="5" style="{{ $font18 }}"><b> การยาสูบแห่งประเทศไทย </b></td>
                                <td width="33%"  align="right" valign="top"  style="{{ $font18 }}"><b> วันที่พิมพ์ : </b> {{ $sysdate }} </td>
                            </tr>
                            <tr>
                                <td width="33%"  align="left" style="{{ $font18 }}"></td>
                                <td width="33%"  align="center" colspan="5" style="{{ $font18 }}"><b> รายงานรายละเอียดวัตถุดิบใช้ไปจริงรวม ตามผลิตภัณฑ์ </b></td>
                            ...
                            <tr>
                                ...
                                <td width="33%"  align="center" colspan="5" style="{{ $font18 }}"><b> {{ ctDateText($dateFrom , $dateTo) }}  </b></td>
                                <td width="33%"  align="center" style="{{ $font18 }}"> </td>
                            </tr>
                            <tr>
            ...
            <tr >
                ...
                <td  align="center"  style="{{ $font18 }}"> จำนวนที่ผลิต :  {{ number_format($groupProducts[0]->total_product_qty ?? 0, 2) }} </td>
            </tr>
            <tr>
            ...
            @foreach ($groupProducts->groupBy('item_number') as $itemNumber => $items)
                <tr>
                    ...
                    <td style="{{ $font16 }}" align="right"> {{ ($items->sum('transaction_quantity')) }}</td>
                    <td style="{{ $font16 }}" align="right" > {{ ($items->first()->transaction_cost) }} </td>
                    <td style="{{ $font16 }}" align="right">{{  ($items->sum('transaction_amount')) }} </td>
                    <td >
                    </td>
                </tr>
            @endforeach
            <tr >
                ...
                <td style="{{ $styleBorderTBFont16 }}  border-top:  1px solid black!important; border-bottom:  1px solid black!important;" align="right"> <b>{{ ($groupProducts->sum('transaction_amount')) }} </b></td>
                <td style="{{ $styleBorderTBFont16 }}  border-top:  1px solid black!important; border-bottom:  1px solid black!important;"></td>
            </tr>
            @foreach ($groupProducts->groupBy('tobacco_group_code') as $groupCode => $dataBygroup)
                <tr>
                    ...
                    <td style="{{ $font16 }}" align="right"> <b> {{ ($dataBygroup->sum('transaction_quantity')) }}</b> </td>
                    <td style="{{ $font16 }}" align="right" ><b>  {{ ($dataBygroup->sum('transaction_amount')) }} </b></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            ...
            @if ($loop->last && count($datas))
                <tfoot>
                    <tr>
                        ...
                        <td style="{{ $styleBorderTBFont16 }} border-top:  1px solid black !important; border-bottom:  1px solid black!important;"><b></b></td>
                        <td style="{{ $styleBorderTBFont16 }} border-top:  1px solid black !important; border-bottom:  1px solid black!important;"></td>
                        <td style="{{ $styleBorderTBFont16 }} border-top:  1px solid black !important; border-bottom:  1px solid black!important;" align="right"> <b>{{ ($datas->sum('transaction_amount')) }} </b></td>
                        <td style="{{ $styleBorderTBFont16 }} border-top:  1px solid black !important; border-bottom:  1px solid black!important;"></td>
                    </tr>
