...
@foreach ($datas->sortBy('product_item_number')->groupBy('product_item_number') as $productItemNumber => $groupProducts)
    <table>
        ...
        <tbody>
            ...
            @foreach ($groupProducts->sortBy('tobacco_group_code')->groupBy('tobacco_group_code') as $groupCode => $dataBygroup)
                <tr>
        ...
        @if ($loop->last && count($datas))
            <tfoot>
                <tr>
                    <td style="{{ $styleBorderTBFont16 }}" colspan="3" align="center"> <b> รวมตามศูนย์ต้นทุน : {{ $cost->cost_code }} {{ $cost->description }}</b> </td>
                    <td style="{{ $styleBorderTBFont16 }}"><b></b></td>
                    <td style="{{ $styleBorderTBFont16 }}"></td>
                    <td style="{{ $styleBorderTBFont16 }}" align="right"> <b>{{ number_format($datas->sum('transaction_amount') ?? 0, 2) }} </b></td>
                    <td style="{{ $styleBorderTBFont16 }}"></td>
                </tr>
                @foreach ($datas->sortBy('tobacco_group_code')->groupBy('tobacco_group_code') as $groupCode => $dataBygroup)
                    <tr>
                        <td style="{{ $font16 }}"> <b> {{$groupCode}} </b>  </td>
                        <td style="{{ $font16 }}"> <b> {{ $dataBygroup[0]->tobacco_group }} </b></td>
                        <td style="{{ $font16 }}" align="right"> <b> {{ number_format($dataBygroup->sum('transaction_quantity') ?? 0, 6) }}</b> </td>
                        <td style="{{ $font16 }}" align="right" ><b>  {{ number_format($dataBygroup->sum('transaction_amount'), 9) }} </b></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
