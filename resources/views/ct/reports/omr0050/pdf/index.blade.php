            ...
            <tbody>
                ...
                @foreach ($dataGroupProduct->sortBy(function($i) {
                    ...
                    @php
                        ...
                        //     $query_gain_loss = \DB::select("
                        //         select  
                        //          round(payment_amount * p.conversion_rate, 2) as a , - round(grand_total * pick_exchange_rate, 2) as b
                        //         from ptom_proforma_invoice_headers h
                        //             ,ptom_payment_exp_v p
                        //         where  h.sa_number = p.prepare_order_number
                        //         and pick_release_no = '$item->payment_number'
                        //         and H.pick_release_status = 'Confirm'
                        //     ");
                            
                        //     $query_gain_loss = collect($query_gain_loss);
                        //     $gain_loss = count($query_gain_loss) ? $query_gain_loss->sum('a') + $query_gain_loss->first()->b : 0;
                        ...
                        $set3 = $raw_gain_loss['result3']->where('sa_number', $item->sa_number)->where('pick_release_no', $item->payment_number)->sum('sub_total') ?? 0;
                        $set4 = $raw_gain_loss['result4']->where('sa_number', $item->sa_number)->where('pick_release_no', $item->payment_number)->sum('exchange');
                        $check_set4 = $raw_gain_loss['result4']->where('sa_number', $item->sa_number)->first();
