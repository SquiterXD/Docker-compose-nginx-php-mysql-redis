<table>             
    <thead>
        <tr>
            ...
            <th width="10" rowspan="3" style="height: 30px; background-color: #ffffff; border: 1px solid #000000; border-bottom: 1px solid #000000; font-weight: bold; font-size: 13px; font-size: 14px; text-align: center;"> 
                หน่วยนับ
            </th>
            <th width="20" rowspan="3" style="height: 30px; background-color: #ffffff; border: 1px solid #000000; border-bottom: 1px solid #000000; font-weight: bold; font-size: 13px; font-size: 14px; text-align: center;"> 
                <p> ปริมาณผลผลิต </p>
        ...
        @foreach($reportLv1Items as $index => $reportLv1Item)
            ...
            <?php 
                ...
                foreach($reportLv2Items as $reportLv2Item) {
                    if($reportLv2Item["product_item_number"] == $reportLv1Item["product_item_number"]) {
            ...
            @foreach($filteredReportLv2Items as $index => $filteredReportLv2Item)
                ...
                <?php 
                    ...
                    foreach($reportItems as $reportItem) {
                        if($reportItem["product_item_number"] == $filteredReportLv2Item["product_item_number"] && $reportItem["wip_step"] == $filteredReportLv2Item["wip_step"]) {
                            $filteredReportItems[] = $reportItem;
                        }
                    }
                    if (count($reportItems)) {
                        $reportItems = collect($reportItems)->sortBy('item_number')->toArray();
                        $reportItems = array_values($reportItems);
                    }
