            ...
            <div class="page-break">
                ...
                <table class="table">
                    <thead>
                            ...
                            <th colspan="{{ $colspan }}">
                                <div class="row">
                                    <div style="width:20%;text-align:left;font-weight:bold;" class="b">
                                        โปรแกรม : {{ $programCode_Output }}<br>
                                        สั่งพิมพ์ : {{ optional(auth()->user())->username }}<br>
                                        <br>
                                        <br>
                                        สถานะ : {{ $comp_status }}<br>
                                    </div>
                        ...
                        <tr
                            style=" background-color: #c4c4c4;border-top: 1px solid #000;border-bottom: 1px solid #000;height:50px;font-weight:bold;">
                            <th
                                style="border-right:1px solid #000;border-left:1px solid #000;width:50px;text-align:center;font-weight:bold;">
                                <b>ลำดับ</b><br>
                            </th>
                            <th style="border-right:1px solid #000;min-width:100px;text-align:center;font-weight:bold;">
                                รายการ<br></th>
                            <th style="border-right:1px solid #000;text-align:center;font-weight:bold;">
                                มูลค่าทุนประกัน<br>(บาท) {{ $comp_percent }}%</th>
                            <th style="border-right:1px solid #000;text-align:center;font-weight:bold;">
                                อัตราเบี้ย<br>ประกัน</th>
                            <th style="border-right:1px solid #000;width:150px;text-align:center;font-weight:bold;">
                                ค่าเบี้ยประกัน<br>{{ $insur_percent }}%<br>(บาท)</th>
                            <th style="border-right:1px solid #000;width:150px;text-align:center;font-weight:bold;">
                                คำนวน<br>ปิดบัญชี<br>(บาท)</th>
                            @foreach ($months as $month)
                                <th style="border-right:1px solid #000;width:150px;text-align:center;font-weight:bold;">
                                    ค่าเบี้ยเฉลี่ย<br>{{ $month->mth_th }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @if (count($company) > 0)
                            @foreach ($company->groupBy('receivable_name') as $receivable => $groups)
                                @if (strlen($receivable) > 0)
                                    <tr
                                        style="background-color:#d0cece; border-bottom:1px solid #000;height:30px;font-weight:bold;">
                                        <td colspan="{{ $colspan }}"
                                ...
                                @foreach ($groups->groupBy('group_01') as $group => $cats)
                                    ...
                                    @foreach ($cats->groupBy('group_02') as $cat => $lines)
                                            ...
                                            @if ($line->cat_title != '')
                                                <tr style="height:25px;">
                                                    ...
                                                    <td style="border-right:1px solid #000;text-align: right;">
                                                        {{ strlen($receivable) > 0 ? ' ' : number_format($line->amt_close, 2) }}
                                                    </td>
                                                    @foreach ($months as $month)
                                                        @php
                                                        ...
                                                        @endphp
                                                        <td style="border-right:1px solid #000;text-align: right;">
                                                            {{ strlen($receivable) > 0 ? ' ' : number_format($line->$m, 2) }}
                                                        </td>
                                                    @endforeach
                                                </tr>
                                            @endif
                                        ...
                                        <!-- sub total -->
                                        <tr
                                            style="background-color:#f3f3f3; border-bottom:1px solid #000;border-top:1px solid #000;height:25px;font-weight:bold;">
                                            <td style="border-right:1px solid #000;border-left:1px solid #000;"></td>
                                            <td
                                                style="border-right:1px solid #000;text-align: center;font-weight:bold;">
                                                รวม</td>
                                            <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                                {{ number_format($lines->sum('amt_cost'), 2) }}</td>
                                            <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                                {{ number_format($lines->max('rate_set'), 5) }}</td>
                                            <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                            ...
                                            @foreach ($months as $month)
                                                <td
                                                    style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                                    {{ strlen($receivable) > 0 ? ' ' : number_format($lines->sum("amt_{$month->mth_code}"), 2) }}
                                                </td>
                                            @endforeach
                                        </tr>
                                    @endforeach
                                ...
                                <tr
                                    style="background-color:#f3f3f3; border-bottom:1px solid #000;border-top:1px solid #000;height:25px;font-weight:bold;">
                                    <td colspan="2"
                                        style="border-right:1px solid #000;border-left:1px solid #000;text-align: center;font-weight:bold;">
                                        รวม : {{ strlen($receivable) > 0 ? $receivable : 'ส่วนกลาง' }}</td>
                                    <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                    ...
                                    @foreach ($months as $month)
                                        <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                            {{ strlen($receivable) > 0 ? ' ' : number_format($groups->sum("amt_{$month->mth_code}"), 2) }}
                                        </td>
                                    @endforeach
                                </tr>
                            @endforeach
                            ...
                            <!--  Grand Total -->
                            <tr
                                style="background-color:#f3f3f3; border-bottom:1px solid #000;height:30px;font-weight:bold;">
                                <td style="border-right:1px solid #000;border-left:1px solid #000;text-align: center;font-weight:bold;"
                                    colspan=2>รวมทั้งสิ้น</td>
                                <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                    {{ number_format($company->sum('amt_cost'), 2) }}</td>
                                <td style="border-right:1px solid #000;text-align: right;font-weight:bold;">
                                ...
                                @foreach ($months as $month)
                                    <td style="border-right:1px solid #000;text-align: right;">
                                        {{ $cnt_null_receivable <= 0 ? ' ' : number_format($company->sum('amt_' . $month->mth_code), 2) }}
                                    </td>
                                @endforeach
                            </tr>
                            @php
