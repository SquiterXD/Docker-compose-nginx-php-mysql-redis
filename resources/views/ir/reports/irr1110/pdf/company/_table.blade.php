...
@foreach ($ptirStockHeader->groupBy('department_code')->sortBy('department_code') as $deptCode => $depts)
    @foreach ($depts->groupBy('sub_inventory_desc')->sortBy('sub_inventory_code') as $subInvDes => $subInvs)
        @foreach ($subInvs->groupBy('location_desc')->sortBy('lcation_code') as $locationDesc => $lines)
            @include('ir.reports.irr1110.pdf._header')
            <table class="table">
                @foreach ($lines as $line)
                    ...
                    @endif
                        <tr>
                            ...
                            <td>{{ $line->item_description }} {{ $line->line_type == 'Manual' ? '(Manual)' : ""}} </td>
                    ...
                    @endforeach
                    <tr>
                        <td colspan="5">รวม กลุ่มย่อย :{{ $locationDesc }}</td>
                    ...
                    @if ($loop->parent->last)
                        <tr>
                            <td colspan="5" >รวม หน่วยงาน :{{ $depts->first()->department_description }}</td>
