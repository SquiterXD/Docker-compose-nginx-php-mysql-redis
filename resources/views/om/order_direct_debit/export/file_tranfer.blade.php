...
@section('page-title')
    <h2><x-get-program-code url="/om/order-direct-debit/export/file-tranfer" /> โอนข้อมูล Text File เพื่อส่งธนาคาร</h2>
    <ol class="breadcrumb">
        <li class="breadcrumb-item">
            <a href="/">หน้าแรก</a>
        </li>
        <li class="breadcrumb-item active">
            <strong><x-get-program-code url="/om/order-direct-debit/export/file-tranfer" /> โอนข้อมูล Text File เพื่อส่งธนาคาร</strong>
        </li>
    </ol>
@stop
                        ...
                        <div class="container form-group form-buttons d-block">
                            <div class="row space-5">
                                <div class="col-md-6 col-6 mb-2">
                                    <button class="btn btn-lg btn-warning m-0 w-100" style="max-width: 100%;" type="button" onclick="genfile()"><i class="fa fa-pencil-square-o"></i> จัดเตรียมข้อมูล</button>
                                </div>
                                <div class="col-md-6 col-6 mb-2">
...
<script>
    ...
    function genfile() {
        ...
        actions_url = "{{ url('/') }}/om/order-direct-debit/export/prepare-file-tranfer"
        $('#bt_submit').click();
    }
    ...
    function download_file() {
        ...
        actions_url = "{{ url('/') }}/om/order-direct-debit/export/confirm-file/"+batch_no+"?count_download="+count_download
        $('#bt_submit').click();

        // window.location.href = "{{ url('/') }}/om/order-direct-debit/export/download_files/"+file_name;
    }
                        ...
                        if(actions_type == 'gen'){
                            if(result.data.length > 0){
                                ...
                                $.each(result.data, function( key, obj ) {
                                    ...
                                    var order_date = obj.order_date.split(' ');
                                    order_date = order_date[0].split('-');

                                    var approve_date = obj.approve_date.split(' ');
                                    approve_date = approve_date[0].split('-');

                                    htmlTb += '<tr class="align-middle">';
                                        htmlTb += '<td>'+(key+1)+'</td>';
                                        htmlTb += '<td>'+obj.prepare_order_number+'</td>';
                                        htmlTb += '<td>'+(order_date[2])+'/'+(order_date[1])+'/'+(parseInt(order_date[0])+543)+'</td>';
                                        htmlTb += '<td>'+obj.customer_name+'</td>';
                                        htmlTb += '<td>'+obj.bank_account_name+'</td>';
                                        htmlTb += '<td>'+obj.toat_bank_account_name+'</td>';
                                        htmlTb += '<td>'+formatMoney(obj.direct_debit_amount,2)+'</td>';
                                        htmlTb += '<td>'+(approve_date[2])+'/'+(approve_date[1])+'/'+(parseInt(approve_date[0])+543)+'</td>';
                                    htmlTb += '</tr>';
                                });
