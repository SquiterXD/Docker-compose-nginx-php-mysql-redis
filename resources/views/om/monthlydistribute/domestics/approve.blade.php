...
<x-get-page-title :menu="$menu" url="" />
    ...
    {{-- <h2> OMP0058 บันทึกประมาณการจำหน่ายรายเดือน</h2>
    <ol class="breadcrumb">
        <li class="breadcrumb-item">
            <a href="/">Home</a>
        </li>
        <li class="breadcrumb-item active">
            <strong> OMP0058 บันทึกประมาณการจำหน่ายรายเดือน</strong> --}}
...
@section('content')
    <div class="ibox">
        <div class="ibox-title">
            {{-- <h5>บันทึกประมาณการจำหน่ายรายเดือน</h5> --}}
        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="input-icon">
                                    <input type="text" list="year_list" class="form-control"  name="budget_year" id="budget_year" onkeyup="this.value = this.value.replace(/[^0-9\.]/g,'');version_import(this.value);" onchange="version_import(this.value);" placeholder="">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-icon">
                                    <input type="text" list="version_list" class="form-control"  name="version" id="version"  placeholder="" onkeyup="this.value = this.value.replace(/[^0-9\.]/g,'');">
...
<script>
    ...
    function searchMonthly()
    {
        ...
        loading();
        $.ajax({
            url : '{{ url("om/ajax/monthly-distribute/searchmonthly") }}',
            type : 'post',
            data : {
                '_token'            : '{{ csrf_token() }}',
            ...
            success:function(rest){
                ...
                if(data.status){
                        ...
                        $.each(data.data.month_header_sort,function(val_head_key,val_head_list){
                            $.each(val_head_list,function(key_month,val_month){
                                if(show == 'quantity'){  
                                    ...
                                    if(data.data.approve == 'อนุมัติ' || data.data.approve == 'ส่งข้อมูลแล้ว'){
                                        if(data.data.data[val_item.item_code][key_month]){
                                            item    += '<span><input type="text" disabled class="form-control text-right" style="background-color: #1c84c6;color: white;" value="'+data.data.data[val_item.item_code][key_month].quantity+'"></span>';
                                        }else{
                                            item    += '<span> 0 </span>';
                                        }
                                    }else{
                                        if(data.data.data[val_item.item_code][key_month]){
                                            item    += '    <span> <input type="text" onkeypress="return isNumber(event);" class="form-control text-right qunti_'+data.data.data[val_item.item_code][key_month].monthly_id+'" data-id="'+data.data.data[val_item.item_code][key_month].monthly_id+'" data-roll="'+item_roll+'_e" onkeyup="inputChange('+col_i+')" data-value="'+data.data.data[val_item.item_code][key_month].type+'"  id="input_col_'+col_i+'_e" name="input_col_qunti['+data.data.data[val_item.item_code][key_month].monthly_id+']" value="'+data.data.data[val_item.item_code][key_month].quantity+'" ></span>';
                                ...
                                // ---------------------------------------------------------------------------------------------------
                                    ...
                                    if(data.data.approve == 'อนุมัติ' || data.data.approve == 'ส่งข้อมูลแล้ว'){
                                        if(data.data.data[val_item.item_code][key_month]){
                                            item    += '<span><input type="text" disabled class="form-control text-right" style="background-color: #1c84c6;color: white;" value="'+data.data.data[val_item.item_code][key_month].amount+'"></span>';
                                        }else{
                                            item    += '<span> 0 </span>';
                                        }
                                    }else{
                                        if(data.data.data[val_item.item_code][key_month]){
                                            item    += '    <span> <input type="text" onkeypress="return isNumber(event);" class="form-control text-right amount_'+data.data.data[val_item.item_code][key_month].monthly_id+'" data-id="'+data.data.data[val_item.item_code][key_month].monthly_id+'" data-roll="'+item_roll+'_e" onkeyup="inputChangeAmount('+col_i+')" data-value="'+data.data.data[val_item.item_code][key_month].type+'"  id="input_col_amount_'+col_i+'_e" name="input_col_amount['+data.data.data[val_item.item_code][key_month].monthly_id+']" value="'+data.data.data[val_item.item_code][key_month].amount+'" ></span>';
                    ...
                    $.each(data.data.producttype,function(key_type,val_type){
                        ...
                        var all_amount = 0;
                        item    += '<tr>';
                        item    += '<td class="text-left" colspan="1" style="background-color: #f5f5f5"><strong class="black"> ยอดรวม '+lebalTotal+' '+ val_type.description +'</strong></td>';
    ...
    function approveMonthly()
    {
        ...
        loading();
        $.ajax({
            url         : '{{ url("om/ajax/monthly-distribute/approved") }}',
            type        : 'post',
            data        : formdata,
            dataType    : 'json',
            cache       : false,
    ...
    {
        var data = [];
        var roll = $("#input_col_"+col_index+"_e").data('roll');
    ...
    function inputChangeAmount(col_index)
    {
        ...
        var a_totalroll = 0;
        $.each($("input[name^=input_col_amount][data-roll^="+roll+"_e]"),function(idx,elem){
            var value_r = $(elem).val().toString().replace(/,/g, '');
    ...
    function sendtoFactory()
    {
        ...
        loading()
        $.ajax({
            url     : '{{ url("om/ajax/monthly-distribute/sendfactory") }}',
            type    : 'post',
            data    : {
                '_token'            : '{{ csrf_token() }}',
                'budget_year'       : budget_year,
    ...
    function saveEditData(id,quantity)
    {
        ...
        loading();
        $.ajax({
            url : '{{ url("om/ajax/monthly-distribute/savevalue") }}',
            type : 'post',
            data : {
                "_token"    : "{{ csrf_token() }}",
                id          : id,
