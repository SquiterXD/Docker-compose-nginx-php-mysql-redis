...
<html>
    <head>
        ...
        @include('om.reports.OMR0064._style')
    </head>
    <body>
        @php
            $total_dr_amount = 0;
        ...
        @foreach ($data as $page => $item)
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach ($item['lines'] as $account => $groupDesc)
                        @foreach ($groupDesc as $desc => $line)
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    {{ $line['debit_amount'] ? numberFormatDisplay($line['debit_amount'], 2) : '' }}
                                </td>
                                <td class="text-right">
                                    {{ $line['credit_amount'] ? numberFormatDisplay($line['credit_amount'], 2) : '' }}
                                </td>
                                <td>
                        ...
                        @if($loop->last)
                            @if($item['end_flag'])
                                <tr style="border-top: 0.5px solid rgb(100, 100, 100);
                                    ...
                                    <td class="text-right">
                                        {{ numberFormatDisplay($total_dr_amount, 2) }}
                                    </td>
                                    <td class="text-right">
                                        {{ numberFormatDisplay($total_cr_amount, 2) }}
                                    </td>
                                    <td>
                                    </td>
                                </tr>
