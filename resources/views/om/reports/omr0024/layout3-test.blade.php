...
<body>
    ...
    <table border="0" width="100%" style="font-size: 12px;">
        ...
        <!-- ****  ไม่อบจ ****  ** กทม. (MT)  ** sec a -->
            <tbody>
                <tr>
                    <td><b>***ไม่อบจ***</b><br/>** กทม. (MT)  **</td>
                @if($cut_item > 0)
                    <td colspan="{{(count($cigarette)-14)+count($rework)+5}}">&nbsp;</td>
                @endif
                </tr>
                    @foreach($layout3_seca as $row)
                        <tr>
                            ...
                            <th>ส่ง<br/>สั่ง</th>
                                @foreach ($cigarette as $item)
                                    ...
                                    @endphp
                                    @foreach ($layout3_seca_convert_rate as $item_lscr)
                                    ...
                                    @if($cut_item == 0)
                                        @if($loop->index <= 13)
                                <td class="text-right" >{{ $row->{'a'.$item->item_id} ? number_format($row->{'a'.$item->item_id} * $uomcode['unit'],2) : '' }}<br/>{{ $row->{'o'.$item->item_id} ? number_format($row->{'o'.$item->item_id},2) : '' }}</td>
                                        @endif
                                    @else
                                        @if($loop->index > 13)
                                <td class="text-right" >{{ $row->{'a'.$item->item_id} ? number_format($row->{'a'.$item->item_id} * $uomcode['unit'],2) : '' }}<br/>{{ $row->{'o'.$item->item_id} ? number_format($row->{'o'.$item->item_id},2) : '' }}</td>
                ...
                <!-- รวมยอด ไม่อบจ. sum seca -->
                @if(count($layout3_sum_seca) > 0)
                    @foreach($layout3_sum_seca as $row)
                        <tr>
                            <td>&nbsp;<b> รวมยอด ไม่อบจ.</b></td>
                            <td>&nbsp;</td>
                            <th>ส่ง<br/>สั่ง</th>
                            @foreach ($cigarette as $item)
                                @if($cut_item == 0)
                ...
                @else
                    <tr>
                        <td style="white-space: nowrap;"><b>&nbsp; รวมยอด ไม่อบจ. </b></td>
                        @if($cut_item > 0)
                        <td colspan="{{(count($cigarette)-14)+count($rework)+5}}">&nbsp;</td>
                        @endif
                    </tr>
        ...
        <!-- ****  อบจ **** ** ต่างจังหวัด (MT) ** sec b -->
            <tbody>
                <tr>
                    <td><b>*** อบจ ***</b><br/>** ต่างจังหวัด (MT) **</td>
                    @if($cut_item > 0)
                    <td colspan="{{(count($cigarette)-14)+count($rework)+5}}">&nbsp;</td>
                    @endif
                </tr>
                @foreach($layout3_secb as $row)
                <tr>
                    <td>{{$row->pick_release_no}}</td>
                    <td  style="white-space: nowrap;" >{{$row->customer_name}}</td>
                    <th>ส่ง<br/>สั่ง</th>
                ...
                <!-- รวมยอด อบจ. sum secb -->
                @if(count($layout3_sum_secb) > 0)
                    @foreach($layout3_sum_secb as $row)
                    <tr>
                        <td  style="white-space: nowrap;" ><b>&nbsp; รวมยอด อบจ.</b></td>
                        <td>&nbsp;</td>
                        <th>ส่ง<br/>สั่ง</th>
                        @foreach ($cigarette as $item)
                            @if($cut_item == 0)
        ...
        <!-- รวมยอดขาย อบจ. และไม่อบจ. sum seca + secb -->
            @if(count($layout3_sum_secab)>0)
                <tbody>
                    @foreach($layout3_sum_secab as $row)
                        <tr>
