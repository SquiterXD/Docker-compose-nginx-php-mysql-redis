...
<body>
            ...
            <tr>
                <td valign="bottom"  class="left" width="30%" sylte=" border: 0.1mm solid #000000;">
                    <table>
                        ...
                        <tr>
                            <td align="left">โปรแกรม : {{$programCode}}_3 </td>
                        </tr>
                        <tr>
    ...
    @foreach($column_header_span as $page_no=> $span_arr)
        ...
        <table border="0" width="100%" style="font-size: 12px;">
            <thead>
                <tr>
                    <th class="text-center" rowspan="2" style="width:120px;border-top:1px solid #000; border-bottom:1px solid #000">เลขที่ใบกำกับสินค้า</th>
                    <th class="nowarp" rowspan="2" style="white-space: nowrap; text-align: left !important; padding-left:15px;border-top:1px solid #000; border-bottom:1px solid #000">ชื่อร้านค้า</th>
                    <th rowspan="2" style="border-top:1px solid #000; border-bottom:1px solid #000"></th>
                    @foreach($span_arr as $header_name => $span)
                    <th class="text-center" style="border-top:1px solid #000; border-bottom:1px solid #000" colspan="{{  $span['colspan'] }}" rowspan="{{  $span['rowspan'] }}" width="100px">{{$header_name}}</th>
                    @endforeach
                </tr>
                <tr>
                    @if(isset($span_arr['บุหรี่']) ||isset($span_arr['ยาเส้น']))
                        @if(isset($span_arr['บุหรี่']))
                            @for($i=$last_cigarette; $i < $cigarette_cnt; $i++)
                                ...
                                @endphp
                                <th class="text-center"  style="border-bottom:1px solid #000" width="100px">{!! @$item_description[0] !!} <br>{!! @$item_description[1] !!}</th>
                            @endfor
                        @endif
                        @if(isset($span_arr['ยาเส้น']))
                            @for($i=$last_rework; $i < $rework_cnt; $i++)
                                <th class="text-center" style="border-bottom:1px solid #000" width="100px">{{ $rework[$i]->item_description }}</th>
                            @endfor
                        @endif
                    @else
                        <tr>
            ...
            <tbody>
                ...
                @foreach($lda as $key => $layout_data_detail)
                    ...
                    @if(($layout_data_detail['section_type']) == 'header')
                        <tr>
                            <td><b>***{{$layout_data_detail['type']}}***</b><br />** {{$layout_data_detail['name']}} **</td>
                            <td colspan="{{$colspan+2}}">&nbsp;</td>
                        </tr>
                    @endif
                    <!------- Section Detail -------------------------------------->
                    @if(($layout_data_detail['section_type']) == 'detail')
                        {{-- @foreach($layout_data_detail['sec'] as $row) --}}
                        @php
                    ...
                    <!-------  Section Sum A & B  ไม่อบจ. และ อบจ. -------------------------------------->
