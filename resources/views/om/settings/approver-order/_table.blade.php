        ...
        <tbody>
            @foreach ($approverOrders as $approverOrder)
                <tr>
                    ...
                    <td>{{ $approverOrder->start_date ? parseToDateTh($approverOrder->start_date) : '' }}</td>
                    <td>{{ $approverOrder->end_date   ? parseToDateTh($approverOrder->end_date) : '' }}</td>
                    {{-- <td>{{ $approverOrder->end_date   ? date(trans('date.format-date'), strtotime($approverOrder->end_date)) : '' }}</td> --}}
                    @if (canEnter('/om/settings/approver-order'))
                        <td align="center">
                            <a href="{{ route('om.settings.approver-order.edit', $approverOrder->approver_order_id) }}" class="btn btn-warning btn-xs mr-2" size="small">
