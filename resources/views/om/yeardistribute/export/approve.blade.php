...
@section('page-title')
    ...
    <h2>OMP0098 บันทึกประมาณการจำหน่ายรายปี สำหรับขายต่างประเทศ</h2>
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">
            <strong>OMP0098 บันทึกประมาณการจำหน่ายรายปี สำหรับขายต่างประเทศ</strong>
...
<div class="ibox">
    <div class="ibox-title">
        <h3>บันทึกประมาณการจำหน่ายรายปี สำหรับขายต่างประเทศ</h3>
    </div>
        ...
        <div class="row space-50 justify-content-md-center flex-column mw-xl-1000 mt-md-4">
            ...
            <div class="col-xl-6 m-auto">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="input-icon">
                                <input type="text" list="year_list" class="form-control" autocomplete="off" onchange="version_import();" onkeyup="version_import();"   name="budget_year" id="budget_year" placeholder="" value="">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-icon">
                                <input type="text"  list="year_list_to" class="form-control" autocomplete="off" onchange="version_import();" onkeyup="version_import();"  name="to_year" id="to_year" placeholder="" value="">
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon">
                        <input type="text" list="version_list" class="form-control"  name="version" id="version" autocomplete="off" placeholder="" value="">
...
<script>
    ...
    function searchYear()
    {
        ...
        loading();
        $.ajax({
            url : '{{ url("om/ajax/year-distribute-export/searchyear") }}',
            type : 'post',
            data : {
                '_token'            : '{{ csrf_token() }}',
            ...
            success:function(rest){
                ...
                if(data.status){
                        ...
                        $.each(data.data.year_header,function(key_year,val_year){
                            if(show == 'quantity'){  
                                if(data.data.data[val_item.item_code][key_year].approve == 'Y'){
                                    item    += '<td>';
                                    item    += '<span><input type="text" disabled class="form-control" value="'+data.data.data[val_item.item_code][key_year].quantity+'"></span>';
                                }else{
                                    item    += '<td>';
                                    ...
                                    item    += '<span> <input type="text" onkeypress="return isNumber(event);"  class="form-control qunti_'+data.data.data[val_item.item_code][key_year].yearly_id+'" data-id="'+data.data.data[val_item.item_code][key_year].yearly_id+'" onkeyup="inputChange('+col_i+')" data-value="'+data.data.data[val_item.item_code][key_year].type+'"  id="input_col_'+col_i+'" name="input_col_qunti['+data.data.data[val_item.item_code][key_year].yearly_id+']" value="'+data.data.data[val_item.item_code][key_year].quantity+'" ></span>';
                                }
                                item    += '</td>';
                            //--------------------------------------------------------------------------------------------
                            }else{
                                if(data.data.data[val_item.item_code][key_year].approve == 'Y'){
                                    item    += '<td>';
                                    item    += '<span> <input type="text" disabled class="form-control" value="'+data.data.data[val_item.item_code][key_year].amount+'"></span>';
                                }else{
                                    item    += '<td>';
                                    // item    += '<span onclick="editData('+data.data.data[val_item.item_code][key_year].yearly_id+','+data.data.data[val_item.item_code][key_year].quantity+');" style="cursor: pointer;" >'+ data.data.data[val_item.item_code][key_year].quantity +'</span>';
                                    item    += '<span> <input type="text" onkeypress="return isNumber(event);"  class="form-control amount_'+data.data.data[val_item.item_code][key_year].yearly_id+'" data-id="'+data.data.data[val_item.item_code][key_year].yearly_id+'" onkeyup="inputChangeAmount('+col_i+')" data-value="'+data.data.data[val_item.item_code][key_year].type+'"  id="input_col_amount_'+col_i+'" name="input_col_amount['+data.data.data[val_item.item_code][key_year].yearly_id+']" value="'+data.data.data[val_item.item_code][key_year].amount+'" ></span>';
                                }
                                item    += '</td>';
                            }
                            col_i += 1;
                    ...
                    $.each(data.data.producttype,function(key_type,val_type){
                        var total_col_i = 1;
                        item    += '<tr>';
                        item    += '<td class="text-center" style="background-color: #f5f5f5"><strong class="black"> ยอดรวม '+lebalTotal+' '+ val_type.description +'</strong></td>';
    ...
    function approveYearly()
    {
        ...
        loading();
        $.ajax({
            url         : '{{ url("om/ajax/year-distribute-export/approved") }}',
            type        : 'post',
            data        : formdata,
            dataType    : 'json',
            cache       : false,
    ...
    {
        var data = [];
        $.each($("input[name^=input_col_qunti][id^=input_col_"+col_index+"]"),function(idx,elem){
            var id = $(elem).data('id');
    ...
    {
        var data = [];
        $.each($("input[name^=input_col_amount][id^=input_col_amount_"+col_index+"]"),function(idx,elem){
            var id = $(elem).data('id');
            data[$(elem).data('value')] = 0;
        });
    ...
    function sendtoFactory()
    {
        ...
        loading();
        $.ajax({
            url     : '{{ url("om/ajax/year-distribute-export/sendfactory") }}',
            type    : 'post',
            data    : {
                '_token'            : '{{ csrf_token() }}',
                'budget_year'       : budget_year,
            ...
            success : function(rest){
                ...
                if(data.status){
                    AlertToast('Success','Send to Factory Complete','success');
                    loading();
                    searchYear();
                }else{
                    if(data.type == 'validate'){
    ...
    function saveEditData(id,quantity)
    {
        loading();
        $.ajax({
            url : '{{ url("om/ajax/year-distribute-export/savevalue") }}',
            type : 'post',
            data : {
                "_token"    : "{{ csrf_token() }}",
                id          : id,
    ...
    function version_import(){
        ...
        $.ajax({
            url         : '{{ url("om/ajax/year-distribute-export/versionyear") }}',
            type        : 'post',
            data        : {
                year        : year,
                year_to     : year_to,
