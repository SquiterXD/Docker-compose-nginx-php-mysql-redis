...
@section('page-title')
    ...
    <h2> OMP0096 บันทึกประมาณการจำหน่ายรายปักษ์ สำหรับขายต่างประเทศ</h2>
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">
            ...
            <strong>OMP0096 บันทึกประมาณการจำหน่ายรายปักษ์ สำหรับขายต่างประเทศ</strong>
        </li>
    </ol>
@stop
...
@section('content')
    <div class="ibox">
        <div class="ibox-title">
            <h5>บันทึกประมาณการจำหน่ายรายปักษ์ สำหรับขายต่างประเทศ</h5>
        </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-icon">
                                    <input type="text" list="version_list" class="form-control"  name="version" id="version" placeholder="Ex: 1">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="input-icon">
                                    <input type="text" list="biweek" class="form-control"  name="biweekly_no" id="biweekly_no" placeholder="Ex: 1" value="">
                                    <datalist id="biweek" >
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-icon">
                                    <input type="text" list="biweek_to" class="form-control"  name="biweekly_to" id="biweekly_to" placeholder="Ex: 24" value="">
...
<script>
    ...
    function searchBiWeekly()
    {
        ...
        loading();
        $.ajax({
           url : '{{ url("om/ajax/transfer-bi-weekly-export/searchbiweekly") }}',
           type : 'post',
           data : {
                '_token'            : '{{ csrf_token() }}',
                        ...
                        $.each(val_sort,function(key_item,val_item){
                            ...
                            $.each(data.data.biweekly,function(key_biw,val_biw){
                                ...
                                if(val_biw[val_item.item_code].approve == 'Y'){
                                    ...
                                    item    += '<span onclick="showApprove('+"'"+val_biw[val_item.item_code].approve_date+"'"+')"> <input type="text" disabled class="form-control text-dark" style="background-color: #999999;" value="'+val_biw[val_item.item_code].quantity+'"></span>';
                                }else{
                                    item    += '    <span> <input type="text" onkeypress="return isNumber(event);" class="form-control qunti_'+val_biw[val_item.item_code].sales_forecast_id+'" data-id="'+val_biw[val_item.item_code].sales_forecast_id+'" onkeyup="inputChange('+col_i+')" data-value="'+val_biw[val_item.item_code].type+'"  id="input_col_'+col_i+'" name="input_col['+val_biw[val_item.item_code].sales_forecast_id+']" value="'+val_biw[val_item.item_code].quantity+'" ></span>';
    ...
    function approveBiWeek()
    {
        ...
        loading();
        $.ajax({
            url         : '{{ url("om/ajax/transfer-bi-weekly-export/approved") }}',
            type        : 'post',
            data        : formdata,
            dataType    : 'json',
            cache       : false,
    ...
    function sendtoFactory()
    {
        ...
        loading()
        $.ajax({
            url     : '{{ url("om/ajax/transfer-bi-weekly-export/sendfactory") }}',
            type    : 'post',
            data    : {
                '_token'            : '{{ csrf_token() }}',
                'budget_year'       : budget_year,
            ...
            success : function(rest){
                ...
                if(data.status){
                    AlertToast('Success','Send Factory Complete','success');
                    loading();
                    searchBiWeekly();
                }else{
                    if(data.type == 'validate'){
    ...
    function showApprove(date)
    {
        Swal.fire({
            ...
            html: 'date: '+date,// add html attribute if you want or remove
    ...
    function saveEditData(id,quantity)
    {
        ...
        loading();
        $.ajax({
            url : '{{ url("om/ajax/transfer-bi-weekly-export/savevalue") }}',
            type : 'post',
            data : {
                "_token"    : "{{ csrf_token() }}",
                id          : id,
    ...
    function version_import(year){
        $.ajax({
            url         : '{{ url("om/ajax/transfer-bi-weekly-export/versionbiweek") }}',
            type        : 'post',
            data        : {
                year        : year,
                '_token'    : '{{ csrf_token() }}',
    ...
    function year_import(year){
        $.ajax({
            url         : '{{ url("om/ajax/transfer-bi-weekly-export/yearbiweek") }}',
            type        : 'post',
            data        : {
                year        : year,
                '_token'    : '{{ csrf_token() }}',
