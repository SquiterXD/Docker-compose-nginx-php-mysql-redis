            ...
            <table  class="table" style="margin-bottom: 0px;">
                <thead>
                    <tr class="pdp0008-table-th-detail-color">
                        <th style="min-width: 65px !important; max-width: 65px !important;">
                            <div>Leaf Formula</div>
                        </th>
                        <th style="min-width: 100px !important; max-width: 100px !important;" class="hidden-sm hidden-xs">
                            <div>รายละเอียด</div>
                        </th>
                        <th style="min-width: 65px !important; max-width: 65px !important;">
                            <div>Casing No.</div>
                        ...
                        <th class="text-center" width="130px;" v-if="false">
                        ...
                        <th class="text-center" width="130px;" v-if="false">
                            <div>ราคาปรับลด/เพื่ม (บาท)</div>
                        </th>
                        <th class="text-center" width="130px;" v-if="false">
                            <div>ต้นทุนหลังปรับราคา (บาท)</div>
                        </th>
                        <th width="230px;" class="text-right"></th>
                    </tr>
                </thead>
                <tbody id="accordion">
                    ...
                    <transition
                        enter-active-class="animated fadeInUp"
                        leave-active-class="animated fadeOutDown">
                        <tr v-if="line.is_collapse && line.casing_id && line.casing_items.length && !line.is_deleted">
                            <!-- <td colspan="2"></td> -->
                            <td colspan="5" class="pl-5 pr-5">
                                <table class="table pl-3 table table-bordered">
                                    <thead>
                                    <tr>
                                    ...
                                    <tr v-for="(item, item_idx) in line.casing_items">
                                        ...
                                        <td>
                                            ...
                                            <el-select  v-if="item.is_edit_item" v-loading="item.loading"
                                                ...
                                                <el-option
                                                    v-for="(item, index) in item.casing_item_list"
                                                    :key="item.item_code"
                                    ...
                                    <casing-item-total :items="line.casing_items"></casing-item-total>
                                </table>
                            </td>
                        </tr>
                    </transition>
...
import CasingItemTotal from './CasingItemTotal.vue';
...
export default {
    ...
    methods: {
        ...
        async addNewItem(line) {
            let vm = this;
            let row = line.casing_items.length;
            let newItem = await _.clone(vm.data.new_casing_line);
            vm.$set(line.casing_items, row, newItem);
            vm.setHasChange();
        },
        ...
        async selectCasing(line) {
            ...
            if (casing) {
                ...
                // line.uom                      = casing.uom;
                line.price_adjus = casing.price_adjus ? casing.price_adjus : 0;
                line.price_reduc_increase = casing.price_reduc_increase ? casing.price_reduc_increase : 0;
                line.cost_after_price_adjus = casing.cost_after_price_adjus ? casing.cost_after_price_adjus : 0;
            }
            vm.setHasChange();
            vm.calUnitCostLine(line);
            // vm.validateDup(line);
        ...
        async selectItem(item) {
            let itemData =  await item.casing_item_list.findIndex(o => o.item_code == item.item_code);
                itemData =  item.casing_item_list[itemData];

                item.inventory_item_id        = itemData.inventory_item_id;
                item.blend_no                 = itemData.blend_no;
                item.fm_casing_id             = itemData.fm_casing_id;
                item.casing_id                = itemData.casing_id;
                item.leaf_formula             = itemData.leaf_formula;
                item.item_code                = itemData.item_code;
                item.old_item_code            = itemData.old_item_code;
        ...
        async calUnitCostLine(line) {
            ...
            line.unit_cost = 0;
            if (leaf && casing) {
                line.unit_cost = casing.unit_cost * (leaf.leaf_proportion /100);
            }
        ...
        async calCost(line) {
            console.log("Casing calCost line", line)
            // let unitCost = parseFloat(line.unit_cost ? line.unit_cost : 0);
        ...
        async remoteItemMethod(query, line, item, inputName) {
            ...
            let lineData = _.clone(line);
                lineData.casing_items = [];
                lineData.casing_leaf_formula_list = [];
                lineData.casing_list = [];
            // let data = _.clone(item);
            //     data[inputName] = [];

            let lines = line.casing_items.filter(o => {
                return o.is_deleted == false && item.inventory_item_id != o.inventory_item_id;
            });

            // let except  = [];
            let except  = [... new Set(lines.map(o => o.item_code)) ];
            // if (isNewLine) {
