            ...
            <table  class="table" style="margin-bottom: 0px;">
                ...
                <tbody id="accordion">
                        ...
                        <tr style="background-color: rgb(252, 252, 252);" :data="i" v-if="!line.is_deleted" :key="i">
                            ...
                            <td class="text-right align-middle">
                                ...
                                <button  :class="transBtn.create.class + ' btn-xs tw-w-25 mb-0 btn-outline'" v-if="line.is_collapse"
                                    :disabled="!header.can.edit"
                                    @click.prevent="addNewFlaItem(line)">
                                    <i :class="transBtn.create.icon"></i>
                                    เพิ่มวัตถุดิบ
                                </button>
                    ...
                    <transition
                        enter-active-class="animated fadeInUp"
                        leave-active-class="animated fadeOutDown">
                        <tr v-if="line.is_collapse && line.flavor_id && line.flavor_items.length && !line.is_deleted">
                            <!-- <td colspan="1"></td> -->
                            <td colspan="3" class="pl-5 pr-5">
                                <table class="table table table-bordered">
                                    ...
                                    <tr v-for="(item, item_idx) in line.flavor_items">
                                        ...
                                        <td>
                                            ...
                                            <input type="text" class="form-control" v-if="!item.is_edit_item" :disabled="!header.can.edit"
                                                ...
                                                @click="clickSelcetItem(line, item, i +''+ item_idx, 'flavor_item_list')" >
                                                <!-- @change="selectFlavor(line)" -->
                                            <el-select  v-if="item.is_edit_item" v-loading="item.loading"
                                                :ref="'flavor_item_list_'+ i +''+ item_idx"
                                                ...
                                                <el-option
                                                    v-for="(flavorItem, index) in item.flavor_item_list"
                                                    :key="flavorItem.item_code"
                                                    :label="flavorItem.item_code"
                                                    :value="String(flavorItem.item_code)"
                                                >
                                                    <span style="float: left">{{ flavorItem.label }}</span>
                                                </el-option>
                                            </el-select>
                                    ...
                                    <flavor-item-total :items="line.flavor_items"></flavor-item-total>
                                </table>
                            </td>
                        </tr>
                    </transition>
                    ...
                    <tr v-if="false">
                        ...
                        <th class="text-right font-weight-bold">{{ Number(totalUnitCost).toLocaleString(undefined, {minimumFractionDigits: 6,maximumFractionDigits: 6}) }}</th>
                        <th></th>
                        <th></th>
                        <th class="text-right font-weight-bold">{{ Number(totalCostAfterPriceAdjus).toLocaleString(undefined, {minimumFractionDigits: 6,maximumFractionDigits: 6}) }}</th>
                        <th></th>
                    </tr>
                </tbody>
            </table>
...
import FlavorItemTotal from './FlavorItemTotal.vue';
...
export default {
    ...
    methods: {
        ...
        async addNewFlaItem(line) {
            let vm = this;
            let row = line.flavor_items.length;
            let newFlaItem = await _.clone(vm.data.new_fla_item);
            vm.$set(line.flavor_items, row, newFlaItem);
            vm.setHasChange();
        },
        ...
        async selectItem(item) {
            let itemData = item.flavor_item_list.findIndex(o => o.item_code == item.item_code);
                itemData =  item.flavor_item_list[itemData];

                item.flavor_id = itemData.flavor_id;
        ...
        async remoteItemMethod(query, line, item, inputName) {
            ...
            let lines = line.flavor_items.filter(o => {
