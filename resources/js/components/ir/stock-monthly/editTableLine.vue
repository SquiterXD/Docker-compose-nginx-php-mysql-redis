<template>
  <div>
    <el-form
      ...
      <div class="table-responsive margin_top_20">
        <div style="font-size: 18px">
          &nbsp; กรมธรรม์ชุดที่ {{ poli }} : {{ polides }} &nbsp; ปี :
          {{ showYearBE("year", yearr) }} &nbsp; วัน เดือน ปี ที่ดึงข้อมูล :
          {{ startt }} - {{ endd }}
        </div>
        <br />
      ...
      <div class="mt-2 mb-2">
        <a :href="`/ir/stocks/monthly/export?id=${headerRow.header_id}&program_code=IRP0002`" type="button" class="btn btn-sm btn-info" target="_bank"> <i class="fa fa-print"></i> Export </a>
      </div>
      ...
      <div class="table-responsive">
        ...
        >
          ...
          <template #cell(period_name)="row">
            <div>
              {{ isNullOrUndefined(row.item.period_name) ? showPeriodNameFormat(row.item.period_name) : isNullOrUndefined(row.item.period_name) }}
            </div>
          </template>
          ...
          <template #cell(original_quantity)="row">
            ...
            >
              <currency-input
                ...
                :disabled="false"
                @input="blurOriginalQty(...arguments, row.index, row.item)"
                @blur-currency="blurOriginalQty(...arguments, row.index, row.item)"
              />
            </el-form-item>
          </template>
          <template #cell(unit_price)="row">
            <div v-if="row.item.flag === 'update' || !complete">
              {{ isNullOrUndefined(row.item.unit_price) ? row.item.unit_price : isNullOrUndefined(row.item.unit_price) }}
            </div>
            <el-form-item
              v-else
            ...
            >
              <currency-input
                ...
                :disabled="false"
                @input="blurUnitPrice(...arguments, row.index, row.item)"
                @blur-currency="blurUnitPrice(...arguments, row.index, row.item)"
              />
            </el-form-item>
          </template>
    ...
    <modal-calculation-date
      ...
      :setDefaultEndDateIsLastDateOfMonth="setDefaultEndDateIsLastDateOfMonth"
    />
  </div>
</template>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'line_number', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'status', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'line_type', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'period_name', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'organization_code', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'sub_inventory_code', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'sub_inventory_desc', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'location_code', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'location_desc', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'asset_group_code', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'stock_list_description', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'item_code', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'item_description', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'uom_description', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'original_quantity', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'unit_price', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'line_amount', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'coverage_amount', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'expense_flag', sortable: true, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'action', sortable: false, class:'text-center text-nowrap', tdClass:'align-middle' },
        { key: 'remove', sortable: false, class:'text-center text-nowrap', tdClass:'align-middle' },
      ],
      totalRows: 0,
      currentPage: 1,
      sortBy: '',
  ...
  methods: {
    ...
    clickRow(dataRow, index) {
      ...
      let Row = {
        ...
        calculate_start_date: this.isNullOrUndefined(
          dataRow.calculate_start_date
        ),
    ...
    clickSelectAll() {
      ...
      vm.rowSelected = [];
      vm.rowSelectedId = [];
      let checked = $(`input[name="stock_month_select_all"]`).is(':checked')
    ...
    clickSelectRow(row_id, obj) {
      let vm = this;
      let checked = $(`input[name="stock_month_select${row_id}"]`).is(':checked')
      if (checked) {
    ...
    clickComplete() {
      ...
      this.$refs.save_table_line.validate((valid) => {
        if (valid) {
          ...
          let data = {};
          this.tableLineAll.filter((all, index) => {
            ...
            if(all.line_type.toUpperCase() === "Manual".toUpperCase()){
              data = {
                ...
                period_from:
                  all.flag === "add"
                    ? this.setYearCE("month", all.period_from)
                    : all.period_from,
                period_to:
                  all.flag === "add"
                    ? this.setYearCE("month", all.period_to)
                    : all.period_to,
              };
            }else {
              data = {
          ...
          let params = {
            data: {
              ...
              department_code: this.headerRow.department_code,
              department_desc: this.headerRow.department_desc,
              asset_group_code: this.headerRow.asset_group_code,
              stock_list_description: this.headerRow.stock_list_description,
              total_cover_amount: this.headerRow.total_cover_amount,
              total_insu_amount: this.headerRow.total_insu_amount,
              manual_amount: this.headerRow.manual_amount,
              manual_cover_amount: this.headerRow.manual_cover_amount,
              manual_insu_amount: this.headerRow.manual_insu_amount,
              inventory_amount: this.headerRow.inventory_amount,
              inventory_cover_amount: this.headerRow.inventory_cover_amount,
              inventory_insu_amount: this.headerRow.inventory_insu_amount,
              program_code: "IRP0002",
              // rows: this.form.tableLine,
              rows: rowsManualYearCE,
              // rows: this.tableLineAll,
            },
          };
    ...
    getDataRowsFromLineAmount(dataRows) {
    ...
    showCalLineAmount(dataRow, index) {
      //  console.log('showCalLineAmount dataRow -> ', dataRow);
      let original_quantity = this.form.tableLine[index].original_quantity;
      let unit_price = this.form.tableLine[index].unit_price;
      if (
        original_quantity &&
    ...
    blurOriginalQty(value, dataIndex, row) {
      if (value) {
        // console.log('blurOriginalQty value ->', value);
        row.original_quantity = value;

        // console.log('blurOriginalQty original_quantity ->', row.original_quantity);
        this.showCalLineAmount(row, dataIndex);
    ...
    blurUnitPrice(value, dataIndex, row) {
      if (value) {
    ...
    receivedCompleted(params) {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          this.res_document_number = res.document_number;
          swal({
            title: "Success",
            text: data.message,
            type: "success",
