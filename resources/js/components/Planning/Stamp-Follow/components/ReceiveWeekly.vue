<template>
    <div class="text-right">
        ...
        <vue-numeric v-if="editFlag" style="width: 100%"
                ...
                v-model="line.weekly_receive_qty"
                @change="changeReceiveQty()"
                @blur="changeReceiveQty()"
        ></vue-numeric>
        <div v-else style="width: 100%;"> {{ formatNumber(Number(line.weekly_receive_qty)) }} </div>
    </div>
</template>
<script>
        ...
        watch: {
            'editFlag': function(newValue) {
                if (newValue == false) {
                    ...
                    // Vue.set(this.lineWeeklyDailyEdit, {});
                }
            },
        },
        methods: {
            ...
            changeReceiveQty(){
                ...
                let result = Number(vm.line.weekly_receive_qty) / Number(vm.stamp.stamp_per_roll);
