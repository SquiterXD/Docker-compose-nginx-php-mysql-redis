                            ...
                            <tr :style="line.holiday_flag == 'Y'? 'background-color: #cccccc;'
                                ...
                                <td class="text-right" :style="sel_daily == line.follow_date_format? 'background-color: #9AD9DB; vertical-align: middle;' : 'vertical-align: middle;'" @click="selDailyDate(line.follow_date_format)">
                                    <div style="width: 100px;">
                                        <!-- {{ line.onhand_qty | number2Digit }} -->
                                        {{ onhandQuantity[line.follow_date] | number2Digit }}
                                    </div>
                                </td>
                                ...
                                <td class="text-right" :style="sel_daily == line.follow_date_format? 'background-color: #9AD9DB; vertical-align: middle;' : 'vertical-align: middle;'" @click="selDailyDate(line.follow_date_format)">
                                    <div style="width: 100px;"> {{ balanceOnhandWeekly[line.follow_date] | number0Digit }} </div>
                                </td>
                                <td class="text-right" :style="sel_daily == line.follow_date_format? 'background-color: #9AD9DB; vertical-align: middle;' : 'vertical-align: middle;'" @click="selDailyDate(line.follow_date_format)">
                                    <div style="width: 100px;"> {{ balanceOnhandRoll[line.follow_date] | number2Digit }} </div>
                                </td>
                                <td class="text-right" :style="sel_daily == line.follow_date_format? 'background-color: #9AD9DB; vertical-align: middle;' : 'vertical-align: middle;'" @click="selDailyDate(line.follow_date_format)">
                                    <div style="width: 100px;"> {{ balanceDays[line.follow_date] | number2Digit }} </div>
                                    <!-- <div style="width: 100px;">{{ line.bal_days }}</div> -->
                                </td>
                            </tr>
                        </template>
                        <tr>
                            ...
                            <td class="text-right" style="vertical-align: middle; background-color: #70d200;">
                                <div class="tw-font-bold text-right" style="width: 100%;">
                                    {{ quantity.weekly_receive | number0Digit }}
                                </div>
                            </td>
                            <td class="text-right" style="vertical-align: middle; background-color: #70d200;">
                                <div class="tw-font-bold text-right" style="width: 100%;">
        ...
        computed: {
            weeklyReceive() {
                return this.lines.reduce((accumulator, line) => {
                    this.quantity.weekly_receive = accumulator + Number(line.weekly_receive_qty);
            ...
            calBalanceStampDaily() {
                ...
                vm.lines.filter(function(line, index) {
                    ...
                    // calculate
                    b = (Number(line.onhand_qty) + Number(line.weekly_receive_qty)) - Number(line.issue_qty);
                    // b = Number(line.onhand_qty);

                    // index.set(vm.balanceQuantity, line.follow_date, Number(bal_days));
                    if (index == vm.lines.length -1) {
                        i = (Number(line.onhand_qty) + Number(vm.lines[index].weekly_receive_qty)) - Number(vm.lines[index].issue_qty) - Number(vm.lines[index].damaged_qty) - Number(vm.lines[index].lost_qty);
                        // อัพเดตคงคลังเช้าใหม่ (คงคลังเช้า) โดยคำนวณค่าของวันปัจจุบันวันสุดท้ายของเดือน
                        ...
                        i = (Number(onhand) + Number(vm.lines[index-1].weekly_receive_qty)) - Number(vm.lines[index-1].issue_qty) - Number(vm.lines[index-1].damaged_qty) - Number(vm.lines[index-1].lost_qty);
