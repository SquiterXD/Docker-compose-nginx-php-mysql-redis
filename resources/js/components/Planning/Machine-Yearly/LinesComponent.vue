<template>
    <div>
        <div class="row col-lg-12 m-2 pl-1" v-if="show_flag && header">
            ...
            <div v-if="month_detail && header" class="form-group pl-2 pr-2 mb-0 col-lg-3 col-md-3 col-sm-6 col-xs-6 mt-2">
        ...
        <div v-if="show_flag" v-loading="loading">
            ...
            <template v-else-if="precentHtml != '' && percentCreate < 100">
                <span v-html="precentHtml"></span>
            </template>

            <template v-if="Object.values(lines).length > 0 && percentCreate == 100">
                <div class="row m-2">
                    <div class="col-lg-5 col-md-5 col-sm-4 col-xs-4 mt-2 pl-2">
                        <div class="tw-font-bold tw-uppercase mb-1">
                            <h2 class="tw-font-bold"> 
                                กำลังผลิตรวมทั้งเดือน : {{ totalProductAll | number2Digit }} {{ unit }}
                            </h2>
                        </div>
                    ...
                    <div class="col-lg-7 col-md-7 col-sm-4 col-xs-4 mt-2 text-right">
                        <div class="col-12">
                            <template v-if="header">
                                <div class="text-right">
                                    <ModalOverview v-if="Object.values(lines).length > 0 && percentCreate == 100 && header.product_type != 'KK'"
                                        :header="header"
                                        :yearlyOverview="yearlyOverview"
                                        :omSaleForcastOverview="omSaleForcastOverview"
                                        :workingHour="workingH"
                                        :onhandYear="onhandYear"
                                        :pWorkingDays="workingDays"
                                        :url="url"
                                        :btnTrans="btnTrans"
                                    > </ModalOverview>
                                    <ModalHol v-if="Object.values(lines).length > 0 && percentCreate == 100"
                                    ...
                                    <OMSalesForecast v-if="header.product_type != 'KK'"
                                        :omSalesForecast="html"
                                        :pSaleForecasts="saleForecasts"
                                ...
                                <div class="text-right">
                                    ...
                                    <button v-if="!isControlDisable && isCurrBiweekly" class="btn btn-warning btn-md" style="padding-left: 7px;" @click.prevent="checkUpdateMachine">
                                        <i class="fa fa-calendar"></i> อัปเดตรายละเอียดเครื่องจักร
                                    </button>
                ...
                <template  v-if="!messageCheckMachineHtml">
                    <div class="row m-2">
                        ...
                        <div class="form-group pl-2 pr-2 mb-0 col-lg-4 col-md-4 col-sm-4 col-xs-4" style="margin-top: 32px;">
                            <button v-if="!isControlDisable && isCurrBiweekly" class="btn btn-success btn-sm text-left" @click.prevent="updateDataEfficiencyMachine">
                        ...
                        <div class="form-group pl-2 pr-2 mb-0 col-lg-3 col-md-2 col-sm-4 col-xs-4" style="margin-top: 32px;">
                            <button v-if="!isControlDisable && isCurrBiweekly" class="btn btn-success btn-sm" @click.prevent="updateDataEfficiencyProduct"> 
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" style="position: sticky;">
                            <thead>
                                <tr>
                                    <th class="text-center sticky-col first-col" style="vertical-align: middle; border-bottom: 0px;">
                            ...
                            <tbody>
                                ...
                                <template v-else>
                                    <template v-for="(lineLists, machineGroup) in lines">
                                        <tr>
                                            ...
                                            <tr v-for="(line, indexLine) in lineLists">
                                                ...
                                                <td class="text-right sticky-col th-col" :style="sel_machine == line.machine_name? 'background-color: #9AD9DB; vertical-align: middle;' : 'vertical-align: middle;'" @click="selMachine(line.machine_name)">
                                                   <div style="width: 100%;"> {{ decimal(line.machine_speed) }} </div>
                                                </td>
                                                ...
                                                <template v-for="(plan, index) in resPlanDate">
                                                    <template v-for="(efficiencyProduct, index) in efficiencyFullProducts">
                                                        <template v-if="machineDowntimes[plan.res_plan_date_display][line.machine_name] == 'Y' && plan.res_plan_date_display == index">
                                                            <td class="text-right" style="background-color: #e02b1e;" :title="efficiencyDayFormula" @click="selMachine(line.machine_name)">
                                                        ...
                                                        <template v-else-if="machineMaintenances[plan.res_plan_date_display][line.machine_name] == 'Y' && plan.res_plan_date_display == index">
                                                            <td class="text-right" style="background-color: #ffc107;" :title="efficiencyDayFormula" @click="selMachine(line.machine_name)">
                                                                <div style="width: 70px;"> 0.00 </div>
                                                            </td>
                                                        </template>
                                                        <template v-else-if="plan.holiday_flag == 'Y' && plan.res_plan_date_display == index">
                                                            <td class="text-right" style="background-color: #cccccc;" :title="efficiencyDayFormula" @click="selMachine(line.machine_name)">
                                                        ...
                                                        <template v-else-if="plan.holiday_flag == 'P' && plan.res_plan_date_display == index">
                                                            <td class="text-right" style="background-color: #878788;" :title="efficiencyDayFormula" @click="selMachine(line.machine_name)">
                                                                <div style="width: 70px;"> 
                                                                    {{ efficiencyProduct[line.machine_name] | number2Digit }}
                                                                </div>
                                                            </td>
                                                        ...
                                                        <template v-else-if="plan.holiday_flag != 'Y'
                                                            ...
                                                            <td class="text-right" :title="efficiencyDayFormula" :style="sel_machine == line.machine_name? 'background-color: #9AD9DB;' : ''" @click="selMachine(line.machine_name)">
                                                                <div style="width: 70px;"> 
                                                                    {{ efficiencyProduct[line.machine_name] | number2Digit }}
                                                                </div>
                                                            </td>
                                                ...
                                                <td :style="sel_machine == line.machine_name? 'background-color: #9AD9DB;' : ''" @click="selMachine(line.machine_name)"> </td>
                                            </tr>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="text-right sticky-col t2-col" style="vertical-align: middle; background-color: #70d200;">
                                                <template v-for="res in summary">
                                                    <template v-if="machineGroup == res.machine_group">
                                                        <span class="tw-font-bold"> {{ res.efficiency_machine_per_min | number2Digit }} </span>
                                                    </template>
                                                </template>
                                            </td>
                                            <template v-for="(plan, index) in resPlanDate">
                                                <td v-if="plan.holiday_flag == 'Y' || plan.holiday_flag == 'P'" class="text-right" style="vertical-align: middle; background-color: #70d200;">
                                        ...
                                        <tr>
                                            <td class="text-right sticky-col t1-col" colspan="4" style="vertical-align: middle;"> <strong> รวมผลผลิตที่คาดว่าจะได้ </strong> </td>
                                            <td class="text-right sticky-col t2-col" style="vertical-align: middle; background-color: #70d200;">
                                                <template v-for="res in summary">
                                                    <template v-if="machineGroup == res.machine_group">
                                                        <span class="tw-font-bold"> {{ res.efficiency_machine_per_min | number2Digit }} </span>
                                    ...
                                    <tr>
                                        <td class="text-right sticky-col t1-col" colspan="4" style="vertical-align: middle;"> <strong> รวมกำลังผลิตแต่ละวัน </strong> </td>
                                        <td class="text-right sticky-col t2-col" style="vertical-align: middle; background-color: #70d200;">
                                            <div class="tw-font-bold text-right"> {{ totalMachineAll | number2Digit }} </div>
                                        </td>
                                        <template v-for="(plan, index) in resPlanDate">
            ...
            <template v-else-if="Object.values(lines).length < 0 && loadingHtml == ''">
...
<script>
    ...
    export default {
        components: {
            EffMachine, OMSalesForecast, MachineDowntime, DateRangeDetail, EfficiencyDetail, VueNumeric, ModalHol, ModalOverview
        },
        ...
        watch:{
            ...
            pEditFlag: async function (newValue) {
        ...
        methods: {
            ...
            selMonth(){
                if (this.month == null) {this.month_detail = ''; return;}
                this.monthDetails.filter(item => {
                    if (item.period_num == this.month){
                        this.sel_month = item.thai_month;
                        this.month_detail = item.thai_full_date;
                    }
                });
            ...
            async getMachineDetail() {
                ...
                await axios.post(vm.url.get_machine_detail, {
                    ...
                    .then(res => {
                        ...
                        if (res.data.status == 'S') {
                            vm.header = res.data.header;
                            // vm.note_description = res.data.header.note_description;
                            ...
                            vm.workingDays = res.data.workingDays;

                            // Alert ปรับเปลี่ยนวันหยุดประจำปี 13032022
            ...
            async updateDataEfficiencyMachineByLine(){
                ...
                let res = await this.updateMachineLineTransactions();
                if(res.status == 'S'){
                    swal({
            ...
            async  updateMachineLineTransactions() {
                let data = [];
                await axios
                    .post(`/planning/machine-yearly/`+this.header.res_plan_h_id+`/update-lines`, {
            ...
            async updateDataEfficiencyProduct(){
                ...
                let res = await this.updateMachineTransactions({
                    ...
                    budget_year: this.search.budget_year,
                    product_type: this.search.product_type,
                    period_num: this.month
                });
                if(res.status == 'S'){
                    swal({
            ...
            async  updateMachineTransactions(value) {
                let data = [];
                await axios
                    .post(`/planning/machine-yearly/`+this.header.res_plan_h_id+`/update`, value)
                    .then(res => {
                        data = res.data;
                    })
                    .catch(err => {
            ...
            async updateMachine() {
                ...
                await axios
                .post(vm.url.update_machine_p01, {params})
                .then(res => {
                    var valid = true;
                    if (res.data.status == 'S') {
                        vm.loading = false;
            ...
            async checkPMConfirm() {
                ...
                await axios.get(vm.url.check_pm_confirm_p01, {params})
                .then(res => {
                    vm.loading = false;
                    if (res.data.status == 'S') {
            ...
            async checkMachineDetail() {
                let vm = this;
                let valid = true;
                await axios.get(vm.url.check_machine_detail_p01)
                .then(res => {
                    vm.loading = false;
                    if (res.data.status == 'S') {
