    ...
    <div>
        <form :action="url.getParam" method="get" target="_blank" @submit="checkForm">
            ...
            <div class="row mt-2">
                ...
                <div class="col-6">
                    <el-select v-model="gas_station_type" filterable clearable
                        class="w-100 text-left"
                        :popper-append-to-body='false'
                        :disabled="!this.group_code"
                    >
                        <el-option
                            v-for="(list, index) in gasStationTypeList"
                            :key="index"
                            :label="list.label"
                            :value="list.value">
                        </el-option>
                        <input type="hidden" name="gas_station_type" :value="gas_station_type">
                    </el-select>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        getGasStationType(){
            ...
            .then(res => {
                this.gasStationTypeList  = res.data.data;
            })
            .catch((error) => {
            })
        },
