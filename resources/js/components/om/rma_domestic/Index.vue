...
<!--                                        <li><a @click="onTestUom">ทดสอบ</a></li>-->
                                    </ul>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    >
                                        <el-option v-for="item in this.filterNumber"
                                                   ...
                                                   :selected="item.rma_number == numberSelected"
                                        >
                                        <div style="display: flex; flex-direction: row; justify-content: space-between;">
                                            <span>{{item.rma_number}}</span>
                                            <span style="font-size: .8rem; color:#ccc">{{ item.rma_number+ ' : ' + item.ref_invoice_number +' : ' + item.status_rma }}</span>
                                        </div>
                                        </el-option>
                                    </el-select>
                                </div>
                            ...
                            <div v-if="isCigaratte" class="table-responsive">
                                <table class="table table-bordered text-center table-hover f13">
                                    ...
                                    <tr v-for="(item,itemIdx) in orderLines" :key="itemIdx">
                                        ...
                                        <td>{{ item.approve_cardboardbox }}</td>
                                        <td>{{ item.approve_carton }}</td>
                                        <td>
                                            <vue-numeric
                                                class="form-control"
                                                separator=","
                            ...
                            <div v-else class="table-responsive">
                                <table class="table table-bordered text-center table-hover f13">
                                    ...
                                    <tr class="align-middle">
                                        ...
                                        <!--จำนวนที่คืน-->
                                        <th class="w-80">จำนวน</th>
                                        <th class="w-80" style="width: 180px">หน่วยนับ</th>
                                        <!-- <th class="w-120">จำนวน</th> -->
                                    </tr>
                                    </thead>
                                    <tbody v-if="orderLines.length">
                                    <tr v-for="(item,itemIdx) in orderLines" :key="itemIdx">
                                        <td>
                                            <input type="checkbox" v-model="enaRow[itemIdx]" :disabled="selNumber.status_rma != 'Draft'">
                                        </td>
                                        <td>{{ itemIdx+1 }}</td>
                                        <td>{{ item.item_code }}</td>
                                        <td>{{ item.item_description }}</td>
...
export default {
    ...
    methods: {
        ...
        async remoteNumbers(query) {
            ...
            await axios.get(vm.url.ajax_get_number_list, { params })
        ...
        async remoteInvoice(query) {
            ...
            await axios.get(vm.url.ajax_get_invoice_list, { params })
            .then(function(response){
        ...
        onAdd() {
            let vm = this;
            vm.dsaNumber = true;
            vm.selNumber.rma_date = moment().add(543, "years").format("DD/MM/YYYY");
        ...
        async onNumberChange() {
            ...
            if (vm.firstSearch) {
                ...
                await vm.remoteInvoice(vm.selNumber.ref_invoice_number)
                const iIdx = await vm.filterInvoice.findIndex(o => o.order_header_id == vm.selNumber.invoice_id);
                        ...
                        axios.patch(vm.url.ajax_create_rma, {
                            ...
                            .then(async response => {
                                if (response.data.result['status'] == "S") {
                                    ...
                                    await axios.get(vm.url.ajax_get_new_number)
                                        .then((response) => {
        ...
        async rmaQuantity(idx){ //Calculate rma_quantity for non cigaratte
            ...
            //     case 'CG': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cg).toFixed(6)
            //         break;
            //     case 'CG2': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cg2).toFixed(6)
            //         break;
            //     case 'CGC': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cgc).toFixed(6)
            //         break;
            //     case 'CGK': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cgk).toFixed(6)
            //         break;
            //     case 'CGP': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cgp).toFixed(6)
            //         break;
            //     case 'CL1': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cl1).toFixed(6)
            //         break;
            //     case 'CS1': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].cs1).toFixed(6)
            //         break;
            //     case 'E00': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].e00).toFixed(6)
            //         break;
            //     case 'KG': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].kg).toFixed(6)
            //         break;
            //     case 'PTN': this.orderLines[idx].rma_quantity = parseFloat(this.orderLines[idx].enter_rma_quantity).toFixed(6) * parseFloat(this.orderLines[idx].ptn).toFixed(6)
            //         break;
