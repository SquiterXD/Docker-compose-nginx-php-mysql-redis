          ...
          <tr v-for="(row, index) in lines">
            ...
            <td>
              <datepicker-th
                  ...
                  @dateWasSelected='endDateWasSelected(...arguments, row)'>
              </datepicker-th>
               <!-- <el-date-picker 
                v-model="row.end_date"
                style="width: 100%;"
      ...
      if (index > 0) {
        var idx = this.lines.find(item => {
          if (item.id != row.id) {
            ...
            if (row.item_id == item.item_id) {
              // console.log('xxx row --->' + row.item_id);
              // console.log('xxx item --->' + item.item_id);
          // console.log('xxx start_date --->' + item.start_date);
          // console.log('xxx end_date --->' + item.end_date);
              if (item.end_date) {
                  // console.log(moment(String(row.start_date)).format('Y-M-d'));
                  // console.log(moment(String(item.end_date)).format('DD-MM-yyyy'));
                // if (row.start_date == '') {
