        ...
        <div class="ibox float-e-margins">
            <div class="ibox-content">
                ...
                <div class="row tw-mb-2">
                    ...
                    <!-- <div class="col-3">
                        <h3 class="no-margins"> ส่งสินค้าสำเร็จรูป </h3>
                    </div> -->
                    ...
                    <div class="col-12 text-right">
                        ...
                        <a :class="reportUrl ? 'btn btn-info btn-lg tw-w-25' : 'btn btn-info btn-lg tw-w-25 disabled'" target="_blank" :href="reportUrl">
                            ...
                            <template v-for="(transferInvItemLine, index) in transferInvItemLines">
                                <tr :key="`${index}`">
                                    ...
                                    <td v-if="isAllowDeleteLine(transferHeader)">
                                        <button v-if="isAllowDeleteLine(transferHeader)" 
                                            type="button" 
...
export default {
    ...
    methods: {
        ...
        setUrlParams() {
            ...
            // queryParams.set("product_date", this.productDateFormatted ? this.productDateFormatted : '');
            queryParams.set("transfer_date", this.transferDateFormatted ? this.transferDateFormatted : '');
            // queryParams.set("to_department_code", this.toDepartmentCode ? this.toDepartmentCode : '');
            queryParams.set("to_locator_id", this.toLocatorId ? this.toLocatorId : '');
            this.setReportUrlParams();
        },
        setReportUrlParams() {
            let vm = this;
            let transferNumber = this.transferNumber ? this.transferNumber : '';
            let transferDateFormatted = this.transferDateFormatted ? this.transferDateFormatted : '';
            let toLocatorId = this.toLocatorId ? this.toLocatorId : '';
        ...
        onTransferInvItemChanged(inventoryItemId, transferInvItemLine, index) {
            ...
            const previousItemNumber = transferInvItemLine.item_number;

            // VALIDATE BEFORE CHANGED

            // ## CASE : NORMAL

            const newItem = this.invItems.find(invItem => {
                return inventoryItemId == invItem.inventory_item_id;
            });
            const foundAvailableItem = this.availableInvItems.find(availableInvItem => {
                return inventoryItemId == availableInvItem.inventory_item_id;
            });

            const foundAvailableItemLines = this.transferInvItemLines.filter(function(o, idx) {
                if (index != idx) {
            ...
            if(foundAvailableItemLines.length > 0) {
                swal("เกิดข้อผิดพลาด", `ไม่สามารถใช้ สินค้าสำเร็จรูป ซ้ำกันได้ ( รหัสสินค้าสำเร็จรูป : ${newItem.item_number})`, "error");
                this.$nextTick(() => {
        ...
        async onAddNewLine() {
            ...
            this.availableInvItems = this.getAvailableInvItems(this.invItems, this.transferInvItemLines);
        ...
        async onDeleteLine(transferInvItemLines, transferInvItemLine, index) {
            ...
            if (index == 0) { // ลบรายการแรก ที่เป็น

                let text = `ต้องการลบรายการ ${transferInvItemLine.item_desc ? transferInvItemLine.item_desc : ""} ?`;
                if (this.transferInvItemLines.length > 1) {
                    text = `ต้องการลบรายการ ${transferInvItemLine.item_desc ? transferInvItemLine.item_desc : ""} ? (หากลบรายการแรกระบบจะทำการลบรายการทั้งหมด)`;
                }
                swal({
                    title: "",
                    text: text,
        ...
        async findHeader() {
            ...
            await axios.get(`/ajax/pm/products/transfers/find-header`, { params })
            .then(res => {
        ...
        async saveHeader() {
            ...
            if(resValidate.valid) {
                ...
                await axios.post(`/ajax/pm/products/transfers/store-header`, reqBody)
                .then(res => {
        ...
        async saveLines() {
            ...
            if(resValidate.valid) {
                ...
                // call store sample result
                await axios.post(`/ajax/pm/products/transfers/store-lines`, reqBody)
                .then(res => {
        ...
        async confirmRequest() {
            ...
            await axios.post(`/ajax/pm/products/transfers/confirm-request`, reqBody)
            .then(res => {
        ...
        async discardConfirmedRequest() {
            ...
            await axios.post(`/ajax/pm/products/transfers/discard-confirmed-request`, reqBody)
            .then(res => {
        ...
        async cancelRequest() {
            ...
            await axios.post(`/ajax/pm/products/transfers/cancel-request`, reqBody)
            .then(res => {
        ...
        async getConfirmItems(showAlert = true) {
            ...
            await axios.get(`/ajax/pm/products/transfers/get-confirm-items`, { params })
            .then(res => {
                ...
                if(resData.status == "success") {
                    ...
                    this.invItems = this.confirmInvItems.map(item => item);
        ...
        async getLines() {
            ...
            await axios.get(`/ajax/pm/products/transfers/get-lines`, { params })
            .then(res => {
        ...
        getAvailableInvItems(invItems, transferInvItemLines) {
        ...
        async checkConversItem() {
            ...
            if (vm.transferInvItemLines.length == 1) { // เมื่อเพิ่มรายการแรก

                console.log("seq: 1", firstLine)
                
                vm.invItems = vm.confirmInvItems.map(item => item);
                vm.availableInvItems = vm.availableInvItems.filter(item => {
                                            return item.convert_flag == firstLine.convert_flag &&
                                                    item.destination_organization_id == firstLine.destination_organization_id
                                        });
                ...
                vm.invItems = vm.invItems.filter(item => {
                                    return item.convert_flag == firstLine.convert_flag &&
                                            item.destination_organization_id == firstLine.destination_organization_id
                                });
            ...
            if (vm.toLocatorId) {
                const foundLocator = vm.inputToLocator.find(o => o.locator_id == vm.toLocatorId);
                if (!foundLocator) {
                    vm.toLocatorId = '';
                }
