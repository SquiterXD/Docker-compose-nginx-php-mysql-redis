    ...
    <div>
        ...
        <div class="ibox float-e-margins">
            <!-- <div class="ibox-content">
                <div class="row">
                    <div class="col-6">
                        <div>availableInvItems</div>
                        <pre>{{ availableInvItems }}</pre>
                    </div>
                    <div class="col-6">
                        <div>availableTransactionItems</div>
                        <pre>{{ availableTransactionItems }}</pre>
                    </div>

                    <div class="col-12">
                        <div>transferInvItemLines</div>
                        <pre>{{ transferInvItemLines }}</pre>
                    </div>
                </div>
            </div> -->
            <div class="ibox-content">
                ...
                <div class="row tw-mb-2">
                    ...
                    <div class="col-12 text-right">
                        ...
                        <modal-search @onSearchRequestHeader="onSearchRequestHeader" :countOpen="countOpenModal" :trans-btn="transBtn" />
                        ...
                        <a v-if="transferObjective && transferObjective >= '40'" :class="reportUrl ? 'btn btn-info btn-lg tw-w-25' : 'btn btn-info btn-lg tw-w-25 disabled'" target="_blank" :href="reportUrl">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="form-group row">
                            ...
                            <label class="col-lg-4 font-weight-bold col-form-label text-right"> สถานะส่งสินค้าสำเร็จรูป : </label>
                        ...
                        <div class="form-group row" v-if="transferObjectives.length > 1">

                            <label class="col-lg-4 font-weight-bold col-form-label text-right"> ประเภทการส่ง : </label>

                            <div class="col-lg-7">

                                <div v-if="isAllowUpdateTransfer(transferHeader) && !this.transferNumber">
                                ...
                                <div v-else>
                                    <input v-if="transferHeader" id="lb-2" type="text" class="form-control" name="transfer_objective"
                                        :value="transferHeader.obj_desc"
                                        :disabled="true"
                                    />
                                </div>
                        ...
                        <div class="form-group row">
                            ...
                            <label class="col-lg-4 font-weight-bold col-form-label text-right required"> สถานที่จัดเก็บปลายทาง : </label>
                            ...
                            <div class="col-lg-7">
                                ...
                                <div v-else>
                                    <input id="lb-2" type="text" class="form-control" name="locator_id"
                                        :value="transferHeader.to_locator_desc"
                                        :disabled="true"
                                    />
                                </div>
                    ...
                    <table class="table table-bordered table-sticky">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th width="310px;" class="text-center">
                                    Lot No.
                                </th>
                                <!-- <th width="140px;" class="text-center" v-if="isAllowUpdateTransfer(transferHeader)">
                                    จำนวน lot
                                </th> -->
                        ...
                        <tbody v-if="transferInvItemLines.length > 0">
                            <template v-for="(transferInvItemLine, index) in transferInvItemLines">
                                <tr :key="`${index}`">
                                    ...
                                    <!-- <td class="align-middle text-right" v-if="isAllowUpdateTransfer(transferHeader)">
...
export default {
    ...
    watch:{
        ...
        inputToLocators : function (val, oldVal) {
            let vm = this;
            let foundLocator = false;
            if (vm.toLocatorId) {
                foundLocator = val.find(o => o.locator_id == vm.toLocatorId);
                if (foundLocator) {
    ...
    async mounted() {
        if (this.transferObjectives.length > 0) {
            this.transferObjective = this.transferObjectiveValue ? this.transferObjectiveValue : this.transferObjectives[0].lookup_code,
            this.transferObjectiveLabel = this.getTransObjectiveLabel(this.transferObjectives, this.transferObjectiveValue ? this.transferObjectiveValue : this.transferObjectives[0].lookup_code)
        }


        this.inputToLocators = [];
        this.setLocators()
    ...
    methods: {
        ...
        setUrlParams() {
            ...
            let vm = this;
            let transferNumber = vm.transferNumber ? vm.transferNumber : '';
            let transferDateFormatted = vm.transferDateFormatted ? vm.transferDateFormatted : '';
            let toLocatorId = vm.toLocatorId ? vm.toLocatorId : '';
            let transferObjective = vm.transferObjective ? vm.transferObjective : '';
            vm.reportUrl = '';
        ...
        onTransferInvItemChanged(inventoryItemId, transferInvItemLine, index) {
            ...
            if(!foundTransactionItem) {
                ...
                this.$nextTick(() => {
                    ...
                    transferInvItemLine.tobacco_group_code          = foundInvItem.tobacco_group_code;
        ...
        onTransactionQtyChanged(transferInvItemLine) {
            ...
            const totalTranQty = filteredTransferItems.reduce((acc, currValue) => acc + currValue.transaction_qty, 0);
            if (totalTranQty > foundInvItem.available_confirm_issue_qty) {
                swal("เกิดข้อผิดพลาด", `รหัสสินค้าสำเร็จรูป : ${foundInvItem.item_number}, ไม่สามารถกรอกผลรวม "จำนวนส่งเข้าคลัง" ได้เกินกว่า ${foundInvItem.available_confirm_issue_qty}`, "error");
                this.$nextTick(() => {
                    transferInvItemLine.transaction_qty = defaultTransactionQty;
                });
                return;
        ...
        async onAddNewLine() {
            ...
            if ((this.transferObjectives.length > 0 && this.transferObjective == '40') && this.transferInvItemLines.length == 0) {
        ...
        setLocators() {
            ...
            let vm = this;
            let inputToLocatorList = false;
            if (vm.transferObjective) {
                let foundTransObjective = vm.transferObjectives.find(item => item.lookup_code == vm.transferObjective);
            ...
            if (vm.inputToLocators.length && vm.profile.organization_code == 'M06') {
                if (vm.transferObjective == '10') { // ส่งเข้าโกดัง
                    vm.inputToLocators = _.filter(vm.inputToLocators, function(o) {
                        return o.locator == 'PURROJ04.99999999';
                    });
                }
                if (vm.transferObjective == '20' || vm.transferObjective == '30') { // 20 ส่งเข้าโรงงาน, 30 ทดลอง
                    vm.inputToLocators = _.filter(vm.inputToLocators, function(o) {
        ...
        async findHeader() {
            ...
            await axios.get(`/ajax/pm/products/trans/find-header`, { params })
            .then(res => {
        ...
        async saveHeader() {
            ...
            if(resValidate.valid) {
                ...
                await axios.post(`/ajax/pm/products/trans/store-header`, reqBody)
                .then(res => {
        ...
        async saveLines() {
            ...
            if(resValidate.valid) {
                ...
                // call store sample result
                await axios.post(`/ajax/pm/products/trans/store-lines`, reqBody)
                .then(res => {
        ...
        async confirmRequest() {
            ...
            await axios.post(`/ajax/pm/products/trans/confirm-request`, reqBody)
            .then(res => {
        ...
        async discardConfirmedRequest() {
            ...
            const confirm = await helperAlert.showProgressConfirm(`ต้องการยกเลิกส่งสินค้าสำเร็จรูป : ${this.transferNumber} ?`);
            if (!confirm) {
                return;
            }
            ...
            await axios.post(`/ajax/pm/products/trans/discard-confirmed-request`, reqBody)
            .then(res => {
        ...
        async cancelRequest() {
            ...
            await axios.post(`/ajax/pm/products/trans/cancel-request`, reqBody)
            .then(res => {
        ...
        async getConfirmItems(showAlert = true) {
            ...
            var params = {
                transfer_date: this.transferDateFormatted,
                objective_code: this.transferObjective,
            };
            await axios.get(`/ajax/pm/products/trans/get-confirm-items`, { params })
            .then(res => {
        ...
        async getLines() {
            ...
            await axios.get(`/ajax/pm/products/trans/get-lines`, { params })
            .then(res => {
        ...
        async checkConversItem() {
            let vm = this;
            let cloneInputToLocators = JSON.parse(JSON.stringify(vm.inputToLocators));
            if (vm.transferInvItemLines.length == 1) {
                let firstLine = await vm.transferInvItemLines[0];
                if (firstLine.item_number) {
                    let filterConfirmInvItems = await this.confirmInvItems.filter(invItem => {
                        return invItem.convert_flag == firstLine.convert_flag &&
                                invItem.destination_organization_id == firstLine.destination_organization_id
                    });
                    let filterTransactionItems = await this.transactionItems.filter(invItem => {
                        return invItem.convert_flag == firstLine.convert_flag &&
                                invItem.destination_organization_id == firstLine.destination_organization_id
                    });;

                    this.invItems = filterConfirmInvItems.filter(invItem => {
                        const foundTransactionItem = this.transactionItems.find(item => invItem.inventory_item_id == item.inventory_item_id);
                        return !!foundTransactionItem;
                    });

                    this.availableTransactionItems = this.getAvailableTransactionItems(filterTransactionItems, this.transferInvItemLines);
                    this.availableInvItems = this.getAvailableInvItems(this.invItems, this.availableTransactionItems);
            ...
            let foundLocator = false;
            if (vm.toLocatorId) {
                const locatorOld = cloneInputToLocators.find(item => item.locator_id == vm.toLocatorId);
                foundLocator = vm.inputToLocators.find(o => o.locator == locatorOld.locator);
                if (foundLocator) {
                    vm.toLocatorId = foundLocator.locator_id;
                }
            }
