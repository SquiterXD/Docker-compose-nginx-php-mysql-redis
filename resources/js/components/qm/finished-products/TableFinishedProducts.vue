    ...
    <div class="table-responsive" style="max-height: 1000px;">
        ...
        <table class="table table-bordered table-sticky mb-0" style="min-width: 2300px;">
            ...
            <thead>
                <tr>
                    <th rowspan="2" class="freeze-col" style="min-width: 300px;"> 
                        <div class="freeze-col-content">
                            <div class="text-center tw-text-xs tw-inline-block tw-align-top tw-py-6" style="min-width: 60px; max-width: 60px;">
                                ลำดับที่
                            </div>
                    ...
                    <th rowspan="2" width="5%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        เขตตรวจคุณภาพ
                    </th>
                    <th rowspan="2" width="5%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        เลขชุดเครื่องจักร
                    </th>
                    <th rowspan="2" width="8%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        วันที่เก็บตัวอย่าง
                    </th>
                    <th rowspan="2" width="7%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        เวลาที่เก็บตัวอย่าง
                    </th>
                    <th rowspan="2" width="12%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        สถานะการลงผล
                    </th>
                    <th rowspan="2" width="12%" class="text-center tw-text-xs pt-3 md:tw-table-cell tw-hidden">
                        สถานะผลการทดสอบ
                    </th>
                    <th rowspan="2" width="13%" class="text-center tw-text-xs md:tw-table-cell tw-hidden" style="min-width: 150px;">
                        สถานะการอนุมัติ
                    </th>
                    <th rowspan="2" width="8%" class="text-center tw-text-xs md:tw-table-cell tw-hidden" style="min-width: 150px;">
                        หมายเหตุ
                    </th>
                    ...
                    <th rowspan="2" width="10%" class="tw-text-xs text-center pt-3 md:tw-table-cell tw-hidden" style="min-width: 140px;"> 
                ...
                <tr>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Minor </th>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Major </th>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Critical </th>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Minor </th>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Major </th>
                    <th width="6%" style="top: 2.55rem;" class="tw-text-xs text-center md:tw-table-cell tw-hidden"> Critical </th>
                </tr>
                ...
                <template v-for="(sampleItem, index) in sampleItems">
                    <tr :key="index">
                        ...
                        <td class="text-center md:tw-table-cell tw-hidden">
                            {{ sampleItem.machine_set }}
                        </td>
                        <td class="text-center md:tw-table-cell tw-hidden">
                            {{ sampleItem.date_drawn_show }}
                        </td>
                        <td class="text-center md:tw-table-cell tw-hidden">
                            <!-- {{ sampleItem.date_drawn_time }}  -->
                            {{ sampleItem.sample_result_test_time }}
                        </td>
                        <td class="text-center md:tw-table-cell tw-hidden">
                            <div v-if="sampleItem.sample_operation_status == 'PD'" class="tw-text-gray-600 tw-rounded tw-p-1 tw-leading-3 tw-font-semibold">
                                <span class="tw-text-2xs"> {{ sampleItem.sample_operation_status_desc }} </span>
                            </div>
                        ...
                        <td class="text-center md:tw-table-cell tw-hidden">
                            <div v-if="sampleItem.sample_result_status == 'PD'" class="tw-text-gray-600 tw-rounded tw-p-1 tw-leading-3 tw-font-semibold">
                                <span class="tw-text-2xs"> {{ sampleItem.sample_result_status_desc }} </span>
                            </div>
                        ...
                        <td class="text-center md:tw-table-cell tw-hidden">
                            <template v-if="showType == 'result' && isAllowEdit(sampleItem, approvalRole)">
                                <textarea rows="2" style="max-height: 70px; height: 70px;"
                                    class="form-control input-sm tw-text-xs text-left"
                                    @blur="onSampleRemarkChanged($event, sampleItem)"
                                    v-model="sampleItem.remark"
                                    placeholder=""
                                />
                            </template>
                            <template v-else>
                                <pre class="text-left tw-bg-white tw-border-none tw-text-xs">{{ sampleItem.remark }}</pre>
                            </template>
                        </td>
                        <td class="text-center md:tw-table-cell tw-hidden">
                    ...
                    >
                        <td colspan="17">
                            <qm-table-finished-product-results
                                :auth-user="authUser"
                                :show-type="showType"
                                :list-brands="listBrands"
                                :list-test-serverity-codes="listTestServerityCodes"
                                :approval-data="approvalData"
                                :approval-role="approvalRole"
                                :sample="sampleItem"
                                :items="sampleItem.specifications"
                                :result-header-items="sampleItem.resultHeaders"
...
export default {
    ...
    methods: {
            ...
            this.sampleItems.forEach(async (element, i) => {
                if (element.sample_no === sampleNo) {
                    if (!this.sampleItems[i].specification_showed) {
                        ...
                        this.sampleItems[i].resultHeaders = resGetSampleSpecifications.resultHeaders;
                        this.sampleItems[i].results = resGetSampleSpecifications.results;
                        this.sampleItems[i].resultLines = resGetSampleSpecifications.resultLines;
                        this.sampleItems[i].attachments = resGetSampleSpecifications.attachments;
                        this.sampleItems[i].defaultData = resGetSampleSpecifications.defaultData;
        ...
        getSampleSpecifications(sampleNo, organizationId, inventoryItemId) {
            ...
            return axios
                .get("/qm/ajax/finished-products/get-sample-specifications", {
                    params
                })
                .then(res => {
                    return {
        ...
        onSampleResultSubmitted(data) {
            if(data.status == "success") {
                ...
                const resSampleTestServerityCodeMinor = data.test_serverity_code_minor;
                const resSampleTestServerityCodeMajor = data.test_serverity_code_major;
                const resSampleTestServerityCodeCritical = data.test_serverity_code_critical;
                const resSampleResultTestTime = data.sample_result_test_time;
                this.sampleItems.forEach((element, i) => {
                    ...
                    if(this.sampleItems[i].sample_no == resSampleNo){
                        ...
                        this.sampleItems[i].test_serverity_code_minor = resSampleTestServerityCodeMinor;
                        this.sampleItems[i].test_serverity_code_major = resSampleTestServerityCodeMajor;
                        this.sampleItems[i].test_serverity_code_critical = resSampleTestServerityCodeCritical;
                        this.sampleItems[i].sample_result_test_time = resSampleResultTestTime;
                    }
                });
            }
        },
        ...
        async approveSample(sampleData, approvalRole, approvalProcessLabel) {
            ...
            await axios.post(`/qm/ajax/finished-products/approval/approve`, reqBody)
            .then((res) => {
        ...
        async approveAllSamples(sampleNos, approvalRole, approvalProcessLabel) {
            ...
            await axios.post(`/qm/ajax/finished-products/approval/approve-all`, reqBody)
            .then((res) => {
        ...
        async rejectSample(sampleData, approvalRole, approvalProcessLabel) {
            ...
            await axios.post(`/qm/ajax/finished-products/approval/reject`, reqBody)
            .then((res) => {
        ...
        async returnSample(sampleData, approvalRole, approvalProcessLabel) {
            ...
            await axios.post(`/qm/ajax/finished-products/approval/return`, reqBody)
            .then((res) => {
        ...
        async unapproveSample(sampleData, approvalRole, approvalProcessLabel) {
            ...
            await axios.post(`/qm/ajax/finished-products/approval/unapprove`, reqBody)
            .then((res) => {
        ...
        async onSampleRemarkChanged(e, sampleItem) {
            ...
            // call store cause of remark
            await axios.post(`/qm/ajax/finished-products/remark`, reqBody)
            .then((res) => {
                ...
                if(resData.status == "success") {
                    // swal("Success", `บันทึกข้อมูล หมายเหตุ สำเร็จ (เลขที่การตรวจสอบ	: ${sampleItem.sample_no} )`, "success");
                }

                if(resData.status == "error") {
                    swal("Error", `บันทึกข้อมูล หมายเหตุ ไม่สำเร็จ (เลขที่การตรวจสอบ	: ${sampleItem.sample_no}) | ${resMsg}`, "error");
                }
            ...
            .catch((error) => {
                console.log(error);
                swal("Error", `บันทึกข้อมูล หมายเหตุ ไม่สำเร็จ (เลขที่การตรวจสอบ	: ${sampleItem.sample_no}) | ${error.message}`, "error");
            });
