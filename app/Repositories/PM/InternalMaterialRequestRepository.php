...
class InternalMaterialRequestRepository
{

    const url = '/pm/internal-material-requests?case=2';
    ...
    function search($request)
    {
        ...
        $attribute2 = $request->attribute2 ?? 2;
        ...
        $headers = PtpmRequestHeader::with([
                        ...
                        ->orderByRaw("request_date, request_number")
                        ->limit(50)
                        ->get();

        $headers = $headers->map(function ($row) {
            $row->show_url = route('pm.internal-material-requests.index', ['request_header_id' => $row->request_header_id]);
            return $row;
        });
        if (count($headers)) {
    ...
    function lines($request)
    {
        ...
        if ($request->inventory_item_id && $header->can->edit) {
            $items = PtpmMfgFormulaV::with(['uom'])
                        ...
                        ->when($header->attribute2 == 1, function($q) {
    ...
    function getItemCatCodeList($profile, $request, $case, $header)
    {
        $columns = ['organization_id', 'item_cat_code', 'item_cat_desc'];
        $tableItemCate = (new PtpmItemCatByOrgV)->getTable();
        $tableFormula = (new PtpmMfgFormulaV)->getTable();
