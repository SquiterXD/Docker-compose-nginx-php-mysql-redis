...
class ProductWipRequestRepository
{
    function info($request)
    {
        $profile = getDefaultData('/pm/products/wip-requests');
    ...
    function getLines($request)
    {
        ...
        if ($header->can->edit) {
            ...
            $jobHeaders = PtpmPmp0044ListJobsV::selectRaw("
                            ...
                            ->groupBy(['organization_id', 'inventory_item_id', 'opt_plan_uom', 'batch_id', 'batch_no', 'transaction_date', 'wip_step', 'confirm_qty'])
                            ->get();
