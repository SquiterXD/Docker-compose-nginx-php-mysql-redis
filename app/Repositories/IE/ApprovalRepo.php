...
use App\Models\IE\Approval;
use App\Models\User;
use App\Models\IE\Employee;
use App\Models\IE\PositionInfo;
use App\Models\IE\POHierarchy;
...
class ApprovalRepo
{
    public function approve($parent, $processType, $approverType, $userId = null)
    {
        ...
        $hierarchyName = HierarchyName::find($parent->hierarchy_name_id);
        $currentSeq = HierarchyNamePosition::find($parent->hierarchy_name_position_id); 
        $nextPosition = $currentSeq ? $hierarchyName->namePositions()->where('seq', '>', $currentSeq->seq)->first() : null;
        $currentPosition = optional($currentSeq)->position ?? null;
        ...
        $approval->last_updated_by = $userId;
        $approval->created_by = $userId;
        $approval->hierarchy_name = optional($hierarchyName)->name;
        $approval->hierarchy_position_name = optional($currentPosition)->name;
        $approval->last_approval = !!$nextPosition;
    ...
    public static function getNextApprover($parent,$processType)
    {
        ...
        if($parent->hierarchy_name_id){
            $name = HierarchyName::find($parent->hierarchy_name_id);

            if($parent->hierarchy_name_position_id){
                $currentPosition = HierarchyNamePosition::find($parent->hierarchy_name_position_id);

                $nextPosition = $currentPosition ? $name->namePositions()->where('seq', '>', $currentPosition->seq)->first() : null;
            }else {
                $nextPosition = $name->namePositions()->first();
            }
    
            if($nextPosition){
                $position = $nextPosition->position;
    
                $user = $position->defaultApprover->user;
    
                $result = [
                    'hierarchy_name_position_id' => $nextPosition->hierarchy_name_position_id,
        ...
        $userId = \Auth::user()->user_id;
        if($parent->next_approver_id == 4 || $parent->next_clearing_approver_id == 4 ){
            // $result = ['user_id' => null];
            return null;
        }else $result = ['user_id' => 4];
    ...
    {
        $query = Approval::where('approvalable_id', $parent->cash_advance_id ?? $parent->reimbursement_id)
                        ->where('process_type',$processType);
        if($approverType){
            $query->where('approver_type',$approverType);
        }
