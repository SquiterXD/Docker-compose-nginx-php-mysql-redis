...
use App\Exports\CT\CTR0021Export;

use PDF;
use Carbon\Carbon;

class CTR0021Controller extends Controller
{
	/**
    ...
    public function index(Request $request)
    {
        ...
        // $stdCostYearMainIds = PtctStdcostYearV::select("year_main_id")->groupBy("year_main_id")->pluck("year_main_id");
        ...
        $preAllocateGroupCodes = PtctStdcostYearAccV::getListAllocateGroupCodes()->whereIn('period_year', $stdCostPeriodYears)->where('allocate_group', 'COST')->get();
        $allocateGroupCodes = [];
        foreach($preAllocateGroupCodes as $index => $preAllocateGroupCode) {
            $allocateGroupCodes[$index] = [];
            $allocateGroupCodes[$index]["period_year"] = $preAllocateGroupCode->period_year;
            $allocateGroupCodes[$index]["allocate_group_code"] = $preAllocateGroupCode->allocate_group_code;
            $allocateGroupCodes[$index]["allocate_group_code_value"] = $preAllocateGroupCode->allocate_group_code;
            $allocateGroupCodes[$index]["allocate_group_code_label"] = "";
            if($preAllocateGroupCode->allocate_group == "DEPT" || $preAllocateGroupCode->allocate_group == "COST"){
            ...
            } else if($preAllocateGroupCode->allocate_group == "PRODUCT"){
                $allocateGroupCodes[$index]["allocate_group_code_label"] = $preAllocateGroupCode->allocate_group_code;
            }
        }

        return view('ct.reports.ctr0021.index', compact(
            'searchInputs',
            'stdCostVersions',
            'stdCostPlanVersions',
            'periodYears',
	...
	public function export(Request $request)
    {
        ...
        $authUser = auth()->user();
        $programCode = getProgramCode('/ct/ctr0021');
        ...
        foreach ($stdCodeTargets as $key => $stdCodeTarget) {
            if($stdCodeTarget->allocate_group_code && $stdCodeTarget->account_type && $stdCodeTarget->target_code && $stdCodeTarget->target_account_code) {
                ...
                foreach($costCenters as $costCenter) {
                    if($stdCodeTarget->target_code == $costCenter->cost_code) {
                ...
                if (count($resultSearchAtc) <= 0) {
                    $agcTargetCodes[] = [
                        ...
                        "target_code_desc"          => $targetCostCodeDesc
                    ];
                }
            }
        }
        ...
        return \Excel::download(new CTR0021Export($programCode, $periodYearTh, $versionNo, $planVersionNo, $periodNameFromTh, $periodNameToTh, $allocateGroupCodes, $agcAccountTypes, $agcTargetAccountCodes, $agcTargetCodes, $accountTypes, $reportItems, $reportAccountItems), "{$filename}.xlsx");
