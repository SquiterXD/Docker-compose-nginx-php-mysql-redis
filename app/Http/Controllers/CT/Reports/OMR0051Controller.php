...
class OMR0051Controller extends Controller
{
    protected $titleReport;
    protected $view = 'ct.reports.omr0051.pdf.index';
    protected $programe;
    ...
    public function calLoss($sa_number) {
        ...
        $result3 = collect(DB::select("SELECT	sa_number, ptom_proforma_invoice_headers.grand_total
                                        * ptom_proforma_invoice_headers.pick_exchange_rate exchange,
    ...
    public function subVat($payment_number) {
        $result1 = collect(DB::select("SELECT   payment_number, sum(vat) vat
                    FROM   ptom_payment_exp_v
                    WHERE   payment_number IN ($payment_number)
    ...
    public function OMR0051($programcode, Request $request)
    {
    ...
    public function queryData($programCode, $request){
        ...
        if($programCode == 'OMR0051') {
            ...
            $text1 = 'sum(ev.payment_amount) amount';
            ...
            $text1 = "(select sum(pdd.conversion_amount) from ptom_payment_details pdd where pdd.payment_header_id = ph.payment_header_id) - sum(ev.vat * ev.conversion_rate) as amount";
        ...
        $arrData['data'] = $arrData['data']->map(function($q) use($forward, $programCode, $raw_gain_loss) {
            ...
            foreach($i as $f) {
                ...
                $set3 = $raw_gain_loss['result3']->where('sa_number', $f->sa_number)->where('pick_release_no', $f->payment_number)->sum('exchange') ?? 0;
                $check_set1 = $raw_gain_loss['result1']->where('prepare_order_number', $f->sa_number)->first();
                $check_set4 = $raw_gain_loss['result3']->where('sa_number', $f->sa_number)->first();
            ...
            // dump("$forward_amount - $forward_grand + $rounded_gain_loss");
