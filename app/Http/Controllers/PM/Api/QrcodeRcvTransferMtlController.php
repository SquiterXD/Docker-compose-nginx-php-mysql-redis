    ...
    public function detail()
    {
        ...
        try {
            ...
            if (($step == 1 || $step == 2) && !request()->is_first_load && request()->new_session != 'true') {
                // $data = PtpmMachinePlanHeaderV::whereDate('plan_date', parseFromDateTh(request()->transfer_date))
                //             ->where('organization_id', $orgId)
                //             ->get();
                // dd('zz', request()->all());
                // $data = (new QrCodeRepository)->mtlPlan($orgId, parseFromDateTh(request()->transfer_date));
                $data = (new QrCodeRepository)->mtlPlan($orgId, parseFromDateTh($currentData->transfer_date));
                if (count($data) == 0) {
                    throw new \Exception("ไม่พบข้อมูลวางแผนของวันที่ ". parseFromDateTh($currentData->transfer_date));
                }
            }
            ...
            if ($step > 0) {
                ...
                $html = view('pm.qrcode-rcv-transfer-mtls._detail_html', compact(
