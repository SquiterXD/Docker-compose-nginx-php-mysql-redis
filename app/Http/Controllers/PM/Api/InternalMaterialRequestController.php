...
class InternalMaterialRequestController extends Controller
{
    ...
    {
        $lines = (new InternalMaterialRequestRepository)->lines($request);
        $data = [
            'lines' => $lines,
        ];
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($headerId) {
                ...
                $header->request_type       = 2; // เป็นการสร้างจากหน้า PMP0032
            ...
            $programCode = $profile->program_code ?? 'PMP0032';
            if ($headerId) {
    ...
    {
        try {
            $items = (new InternalMaterialRequestRepository)->getItem($request);
            $data = [
                'status' => 'S',
                'items' => $items
            ];
