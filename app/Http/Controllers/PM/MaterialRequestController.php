...
class MaterialRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        $profile = getDefaultData('/pm/request-materials');
        $headerId   = request()->request_header_id ?? '-1';
        $lines = [];

        $materialRequestRepo = new MaterialRequestRepository;
        ...
        $url = (object)[];
        $url->ajax_lines = route('pm.ajax.material-requests.lines');
        $url->ajax_lines = route('pm.ajax.material-requests.lines');
        $url->ajax_store = route('pm.ajax.material-requests.store');
        $url->ajax_get_item = route('pm.ajax.material-requests.get-item');
        $url->ajax_set_status = route('pm.ajax.material-requests.set-status', $headerId);
        $url->ajax_set_status = route('pm.ajax.material-requests.set-status', $headerId);
        ...
        $url->material_requests_index = route('pm.material-requests.index', ['case' => $header->attribute2]);
        $url->index = route('pm.material-requests.index');
        ...
        return view('pm.material-requests.index', compact(
            "header", "lines", "data",
            'url', 'profile'
        ));
    }
    ...
    {
        try {
            $repo = new MaterialRequestRepository;
