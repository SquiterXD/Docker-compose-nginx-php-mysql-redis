...
class ConversTransactionController extends Controller
{
    public function index()
    {
        ...
        $url = "/pm/convers-transactions";
        $menu = getMenu($url);
        $getProgramCode = getProgramCodeView($menu)->render();
        $url = (new ConversTransactionRepository)->url(request());
        return view('pm.convers-transactions.index', compact('url', 'menu', 'getProgramCode'));
    }

    private function data($request)
    {
        try {
            $repo = new ConversTransactionRepository;
