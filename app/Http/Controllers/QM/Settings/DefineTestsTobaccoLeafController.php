...
class DefineTestsTobaccoLeafController extends Controller
{
    ...
    public function index()
    {
        ...
        $btnTrans = trans('btn');
        return  view('qm.settings.define-tests-tobacco-leaf.index',
    ...
    public function create()
    {
        ...
        $btnTrans = trans('btn'); 
        $testTypeCode = '1';                               
        return  view('qm.settings.define-tests-tobacco-leaf.create',
                compact('units', 'dataTypes', 'resultSeverites', 'btnTrans', 'testTypeCode'));
    }

    public function store(Request $request)
    {
        $profile = getDefaultData('/qm/settings/define-tests-tobacco-leaf');
        ...
        foreach($request->dataGroup as $index=>$data){
            ...
            if($data['data_type_code'] == "N" && $data['decimals'] > 10){
                return redirect()->back()
                                ->withInput($request->all())
                                ->with('error', 'ไม่สามารถกรอกตัวเลขจำนวนเต็ม '. $data['decimals'] . ' ได้ ต้องเป็นตัวเลขจำนวนเต็ม ระหว่าง 0 ถึง 9 เท่านั้น!');
            }
                ...
                if(isset($data['files'])){
                    foreach ($data['files'] as $key => $value) {
                        if($key == '0'){
                            ...
                            $defineTests->enable_flag           = $data['enable_flag'] ? 'Y' : 'N' ;
        ...
        return redirect()->route('qm.settings.define-tests-tobacco-leaf.index')->with('success','ทำการเพิ่มรายการเรียบร้อยแล้ว');
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('qm.settings.define-tests-tobacco-leaf.index')->with('success','เปลี่ยนแปลงข้อมูลเรียบร้อยแล้ว');
    }
}
