...
class DefineTestsFinishedProductsController extends Controller
{
    ...
    {   
        ...
        $tests = TestsV::select(    'test_id', 'test_code', 'test_desc', 'test_unit_code', 
                                    ...
                                    'qm_process_code', 'check_list_code')
                        ->search($search)
        ...
        $processDistinctTestList = \DB::select("
                                        SELECT DISTINCT      
                                            PQTP.LOOKUP_CODE AS PROCESS_CODE,
                                            PQTP.MEANING AS PROCESS_MEANING                                 
                                        FROM
                                            OAQM.PTQM_QUALITY_TESTING_CIGARETTE PQTC,
                                            OAQM.PTQM_QUALITY_TESTING_PROCESS PQTP
                                        WHERE
                                            (PQTC.ATTRIBUTE2 = PQTP.LOOKUP_CODE)
                                        ORDER BY
                                            PQTP.MEANING ASC
                                    ");
        $processTestList = \DB::select("
                                            SELECT       
                                                PQTP.LOOKUP_CODE AS PROCESS_CODE,
                                                PQTP.MEANING AS PROCESS_MEANING,                                 
                                                PQTC.LOOKUP_CODE AS ENTITY_CODE,
                                                PQTC.MEANING AS ENTITY_MEANING
                                            FROM
                                                OAQM.PTQM_QUALITY_TESTING_CIGARETTE PQTC,
                                                OAQM.PTQM_QUALITY_TESTING_PROCESS PQTP
                                            WHERE
        ...
        $btnTrans = trans('btn');
        return  view('qm.settings.define-tests-finished-products.index',
                compact('tests', 'resultSeverites', 'units', 'dataTypes', 
                        'btnTrans', 'search', 'testDescList', 'testTypeCode',
    ...
    public function create()
    {
        ...
        $entityTestList = \DB::select("
                                        ...
                                        ORDER BY
                                            ...
                                            PQTC.MEANING ASC                              
                                    ");
        return  view('qm.settings.define-tests-finished-products.create',
                compact('units', 'dataTypes', 'resultSeverites', 'btnTrans',
                        'processDistinctTestList', 'processTestList', 'entityTestList', 'testTypeCode'));
    }

    public function store(Request $request)
    {
        $profile = getDefaultData('/qm/settings/define-tests-finished-products');
        ...
        foreach($request->dataGroup as $index=>$data){
            if($data['data_type_code'] == "U" || $data['data_type_code'] == "T"){
                $this->validate($request,[
                    ...
                    // 'dataGroup.*.entity'            => 'required',
                    // 'dataGroup.*.process'           => 'required',
                ],[
                    ...
                    // 'dataGroup.*.entity'                    => 'โปรดเลือกรายการตรวจสอบคุณภาพบุหรี่',
                    // 'dataGroup.*.process'                   => 'โปรดเลือกกระบวนการตรวจคุณภาพบุหรี่',
                ]);
            }else {
                $this->validate($request,[
                    'dataGroup.*.test_desc'         => 'required',
                ...
                if(isset($data['files'])){
                    foreach ($data['files'] as $key => $value) {
                        if($key == '0'){
                            ...
                            // $defineTests->attribute4            = $data['process'];
                            // $defineTests->attribute5            = $data['entity'];
        ...
        return redirect()->route('qm.settings.define-tests-finished-products.index')->with('success','ทำการเพิ่มรายการเรียบร้อยแล้ว');
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('qm.settings.define-tests-finished-products.index')->with('success','เปลี่ยนแปลงข้อมูลเรียบร้อยแล้ว');
    }
}
