...
class IRR0003_2Controller extends Controller{
	public function export($programCode, $request){
        $programCode_Output = 'IRR0003-2'; // programCode Show Report
        $programCode_Query = 'IRR0003-2'; // programCode use Query
        ...
        $tb_view="
            SELECT
                ...
                MAX(loc.attribute1) group_01,
                MAX(l.location_name) group_02,
            ...
            JOIN PTIR_LOCATION_V loc on (loc.value = l.location_code)
            $join_company_aggregate
            JOIN PTIR_POLICY_SET_HEADERS h on (h.policy_set_header_id = l.policy_set_header_id)
            JOIN PTIR_GROUPS g on (g.lookup_code = h.group_code)
            ...
            GROUP BY l.POLICY_SET_HEADER_ID, {$comp_code}, {$comp_name}, {$comp_percent}, {$insur_percent}, g.description, l.RECEIVABLE_NAME , l.LOCATION_CODE , l.DEPARTMENT_CODE , l.CATEGORY_CODE, z2.company_code, z2.COMPANY_NAME
            ORDER BY l.POLICY_SET_HEADER_ID, {$comp_code}, {$comp_name}, {$comp_percent}, {$insur_percent}, g.description, NVL(l.RECEIVABLE_NAME, ' '), l.LOCATION_CODE , l.DEPARTMENT_CODE , l.CATEGORY_CODE
        ";
        ...
        // dd($data,$titles,$months,$repTitle,$programCode_Output,$progTitle,$progTitle,$progPara);

        $pdf = PDF::loadView('ir.reports.irr0003_2.index',compact('data','titles','months','repTitle','programCode_Output','progTitle', 'data_diff_array_grand_total', 'progPara'))
