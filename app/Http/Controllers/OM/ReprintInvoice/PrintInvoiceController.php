        ...
        if ($order->count() > 0) {
            ...
            foreach($order as $order_item){
                ...
                $priceUnit = DB::raw("(SELECT operand FROM ptom_price_list_line_v
                    ...
                    AND ptom_price_list_line_v.start_date_active <= TO_DATE('". now()->format('Y-m-d') ."', 'YYYY-MM-DD')
                    and (ptom_price_list_line_v.end_date_active >= TO_DATE('". now()->format('Y-m-d') ."', 'YYYY-MM-DD') or ptom_price_list_line_v.end_date_active is null)
                ) as price_unit");
                ...
                // SHIPMENT BY
                $getShipmentQuery   = ShipmentBy::select('meaning', 'lookup_code')->where('lookup_code', $order_item->transport_type_code)->first();

                if (!empty($getShipmentQuery)) {
                    
                    if ($getShipmentQuery->lookup_code == '20') {
                    $dateNow = Carbon::createFromFormat('d/m/Y', $order_item->delivery_date)->subYears(543)->format('Y-m-d');
                    $transportOwnerName = DB::table('ptom_transport_owners')
                                            ->whereRaw("to_date('{$dateNow}', 'YYYY-MM-DD') BETWEEN start_date AND end_date")
                                            ->pluck('transport_owner_name')
                                            ->first();
                ...
                if (!empty($orderLine)) {
                    foreach ($orderLine as $key => $value) {
                        ...
                        $operand        = PriceListLine::where('list_header_id', $listHeaderID)->where('item_id', $value->item_id)->whereRaw("ptom_price_list_line_v.start_date_active <= TO_DATE('". now()->format('Y-m-d') ."', 'YYYY-MM-DD')
                             and (ptom_price_list_line_v.end_date_active >= TO_DATE('". now()->format('Y-m-d') ."', 'YYYY-MM-DD') or ptom_price_list_line_v.end_date_active is null)")->pluck('operand')->first();

                        // $total_operand  = $value->approve_quantity * $operand;
                        $total_operand  = $value->approve_quantity * $value->attribute1;
                        ...
                        // }else{
                            ...
                            $orderLine[$key]->total_amount  = number_format($value->total_amount, 2, '.', ',');
                            $orderLine[$key]->operand       = number_format($value->attribute1, 2, '.', ',');
                            // $orderLine[$key]->operand       = number_format($operand, 2, '.', ',');
                            $orderLine[$key]->total_operand = number_format($total_operand, 2, '.', ',');
