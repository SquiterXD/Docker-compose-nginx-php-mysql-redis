...
class OMR0032Controller extends Controller
{
...
where
       ...
       --and c.customer_id=108
       and TO_CHAR(
               NVL(S.consignment_date, S.pick_release_approve_date),
               'MM-YYYY'
       ) = '{$month->format('m-Y')}'
union
---2
...
select
       ...
       approve_quantity,
       CASE
               WHEN approve_quantity - round(
                       approve_quantity *(100 - TAX_PAO_PERCENT) / 100,
                       -1
               ) < 0 THEN 0
               ELSE approve_quantity - round(
                       approve_quantity *(100 - TAX_PAO_PERCENT) / 100,
                       -1
               )
       END as P_Qty 
...
where
       ...
       and order_header_id not in (
               select
                       distinct order_header_id
               from
                       ptom_pao_percent_inv
       )
       union
       select
               C.customer_number,
       ...
       ";
        ...
        // return view('om.reports.omr0032.index',
        // $data);
        $pdf = PDF::loadView(
            'om.reports.omr0032.index',
            $data
        )
