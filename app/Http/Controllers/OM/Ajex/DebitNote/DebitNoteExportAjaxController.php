...
class DebitNoteExportAjaxController extends Controller
{
    ...
    {
        $list  = DebitNoteModel::where('program_code','OMP0076')
                                ->where(function($query) use($request){
    ...
    {
        $order      = ProformaInvoiceModel::where('customer_id',$request->customer_id)
                                        ...
                                        ->orderBy('pick_release_no','DESC')
                                        ->get();
        if($order->count() > 0){

            foreach($order as $item){
                $data[] = [
                    'id'        => $item->pi_header_id,
        ...
        if($validate->fails()){
            ...
            try {
                if(!empty($request->invoice_id)){
                    ...
                    $invoice->currency                  = $request->currency_code;
                    ...
                    $invoice_number = GenerateNumberRepo::generateCreditNoteExportDomestics();
                ...
                if(!empty($request->product)){
                    foreach($request->product as $key_product => $product_item){
                        ...
                        if($invoiceline){
                            ...
                            $invoiceline->currency              = $request->currency_code;
                            ...
                            $invoiceline->program_code          = 'OMP0076';
    ...
    public function searchInvoice(Request $request)
    {
        ...
        if($data){
            ...
            if(!empty($data->document_id)){
                $orderheader    = ProformaInvoiceModel::where('pi_header_id',$data->document_id)->first();
                $datahead = [
        ...
        if($orderheader){
            ...
            foreach($orderline as $key_line => $orderline){
                $data['line'][] = [
                    ...
                    'approve_total'         => !empty($orderline->approve_quantity)? number_format($orderline->approve_quantity,2) : number_format(0,2)
                ];
            }
            $rest = [
                'status'    => true,
