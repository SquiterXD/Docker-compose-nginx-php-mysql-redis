...
class User extends Authenticatable
{
    ...
    public function isChangeCOAUser()
    {
        $changeCOAUsers = \App\Models\IE\Preference::getChangeCOAUsers();
        if(count($changeCOAUsers) > 0){
            return in_array($this->user_id, $changeCOAUsers);
        }
        return false;
    }
    ...
    public function setSession($changeDeptCode = false)
    {
        ...
        $dept = PtglCoaDeptCodeV::where(function($r) use($user, $departmentOptions) {
                        $r->whereIn('department_code', $departmentOptions)
                            ->orWhere('department_code', $user->department_code);
                    })
                    ->selectRaw("ptgl_coa_dept_code_v.*, department_code ||': '|| description display")
                    ->get();
