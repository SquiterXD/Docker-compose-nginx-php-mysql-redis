...
{
    const URL_PATH = '/pd/casing-simu_additive';
    const MOCK_PROGRAM_CODE = 'PD0001';
    ...
    public function store(Request $request)
    {
        ...
        $lines = DB::SELECT("SELECT DISTINCT L.SIMU_FORMULA_LINE_ID,
                                        ...
                                        LEFT JOIN  OAPD.PTPD_RAW_MATE_CASING_V RM
    ...
    public function show($id)
    {
        ...
        $histories = DB::table('ptpd_simu_additive_history')
            ->select('simu_for_history_id', 'edit_by', 'edit_date', 'edit_field', 'edit_no', 'old_data', 'new_data', 'item_code')
            ->where('simu_formula_id', $id)
            ->where('edit_field', '!=', 'actual_cost')
            // ->where('edit_field', '!=', 'description')
