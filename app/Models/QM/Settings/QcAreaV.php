...
class QcAreaV extends Model
{
    use HasFactory;

    protected $table = 'ptqm_qc_area_v';
    ...
    public function scopeSearch($q, $request)
    {
        if ($request){
            ...
            if(array_key_exists('qc_area',$request)){
                if ($request['qc_area'] != null) {
                    $q->where('qc_area', $request['qc_area'])->get();
                } else {
                    $q;
                }  
            }
