...
class QcAreaLeakRateV extends Model
{
    use HasFactory;

    protected $table = 'ptqm_qc_area_leak_rate_v';
    ...
    public function scopeSearch($q, $request)
    {
        if ($request){
            ...
            if(array_key_exists('qc_area_leak_rate',$request)){
                if ($request['qc_area_leak_rate'] != null) {
                    $q->where('qc_area_leak_rate', $request['qc_area_leak_rate'])->get();
                } else {
                    $q;
                }  
            }
